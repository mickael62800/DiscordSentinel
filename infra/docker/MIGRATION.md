...
`infra/docker/`, ou ailleurs), Docker utilisera **toujours** le project name
...
```bash
docker compose -f infra/docker/docker-compose.yml --profile tls up -d
```
...
3. Vérifie que le compose a bien `name: discordsentinel` :
   ```bash
   grep "^name:" infra/docker/docker-compose.yml
...
| #10 docker-bake.hcl | ✅ | `infra/docker/docker-bake.hcl` |
