...
use nexus_core::{domain::errors::DomainError, ports::outbound::coude_inventory_repository::{CoudeInventoryRepository, InventoryItem}};
use sqlx::PgPool;
use super::pg_err;

pub struct PgCoudeInventoryRepository { pool: PgPool }
impl PgCoudeInventoryRepository { pub fn new(pool: PgPool) -> Self { Self { pool } } }
#[async_trait]
impl CoudeInventoryRepository for PgCoudeInventoryRepository {
    async fn list(&self, guild_id: &str, user_id: &str) -> Result<Vec<InventoryItem>, DomainError> {
        let rows: Vec<(String, i32)> = sqlx::query_as("SELECT item_key, quantity FROM nexus_coude_inventory WHERE guild_id=$1 AND user_id=$2 AND quantity > 0 ORDER BY item_key")
    ...
    async fn buy(&self, guild_id: &str, user_id: &str, item_key: &str, price: i64) -> Result<i64, DomainError> {
        ...
        sqlx::query("INSERT INTO nexus_coude_inventory (guild_id,user_id,item_key,quantity) VALUES ($1,$2,$3,1) ON CONFLICT (guild_id,user_id,item_key) DO UPDATE SET quantity=nexus_coude_inventory.quantity+1")
            .bind(guild_id).bind(user_id).bind(item_key).execute(&mut *tx).await.map_err(pg_err)?;
        sqlx::query("INSERT INTO nexus_wallet_transactions (guild_id,user_id,amount,balance_after,source,description) VALUES ($1,$2,$3,$4,'coude_shop','Achat Coup de Coude')")
