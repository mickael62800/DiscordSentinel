...
use nexus_core::{domain::errors::DomainError, ports::outbound::coussin_steal_repository::CoussinStealRepository};
use sqlx::PgPool;
use super::pg_err;

pub struct PgCoussinStealRepository { pool: PgPool }
impl PgCoussinStealRepository { pub fn new(pool: PgPool) -> Self { Self { pool } } }
#[async_trait]
impl CoussinStealRepository for PgCoussinStealRepository {
    ...
    async fn transfer(&self,guild:&str,thief:&str,victim:&str,amount:i64,success:bool,cooldown_minutes:i64)->Result<(),DomainError>{
        ...
        if success { sqlx::query("UPDATE nexus_coussin_players SET total_stolen=total_stolen+$3 WHERE guild_id=$1 AND user_id=$2").bind(guild).bind(thief).bind(amount).execute(&mut *tx).await.map_err(pg_err)?; }
