...
/// Gère l'activation automatique du slowmode pendant un raid.
...
impl SlowmodeManager {
    ...
    pub async fn activate(
        ...
        let channels = match guild_id.channels(&ctx.http).await {
            Ok(c) => c,
            Err(e) => {
                error!(error = %e, "Impossible de récupérer les channels");
                return;
            }
        };
    ...
    pub async fn deactivate_with_http(&self, http: &serenity::http::Http, guild_id: GuildId) {
        ...
        info!(
            ...
            "Slowmode anti-raid désactivé"
        );
    }

    /// Retourne les guilds dont le slowmode a dépassé la durée maximale.
    ...
    /// Vérifie si le slowmode est actif pour un serveur.
