//! MOD #6 — Commande `/template` : gestion des reason templates depuis Discord.
//!
//! Les templates sont stockes dans `bot_guild_config` sous la cle
//! `reason_templates` (bot `moderation-bot`), au format `label|raison` par ligne.
//! Ils alimentent l'autocomplete des champs "reason" des commandes `/warn`,
//! `/mute`, `/ban`. Cette commande permet aux senior mods de les gerer sans
//! passer par le GUI desktop.
//!
//! Sous-commandes :
//!   - `/template list` — affiche les templates actuels
//!   - `/template add <label> <reason>` — ajoute un template
//!   - `/template remove <label>` — supprime un template (par label exact)
//!
//! Gated `ADMINISTRATOR` cote Discord (default_member_permissions). Aucune
//! protection RBAC cote API car le bot n'envoie pas de X-Discord-Token.
...
use crate::reason_templates::{self, ReasonTemplate};
