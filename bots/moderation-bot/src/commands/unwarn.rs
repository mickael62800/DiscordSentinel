...
pub async fn handle(ctx: &Context, command: &CommandInteraction) {
    ...
    // Recuperer l'historique du user
    ...
    // Construire la liste avec un bouton ❌ par warn
    ...
    for (i, w) in warns.iter().enumerate() {
        let date = &w.created_at[..10]; // YYYY-MM-DD
        ...
        // Max 5 boutons par rangee, max 5 rangees = 25 boutons
        if buttons.len() < 25 {
            buttons.push(
    ...
    // Grouper les boutons par rangees de 5
