//! MOD #1 — Commande `/expirations` : liste les sanctions temporaires actives
//! (mute_temp / ban_temp) d'un serveur avec leur temps restant.
//!
//! Source de verite : table `sanction_reminders` exposee via
//! `GET /api/reminders/{guild_id}`. Affiche les rappels `pending` dont
//! `expires_at > NOW()` (sanctions encore actives).
...
pub async fn handle(ctx: &Context, command: &CommandInteraction) {
    ...
    let now = Utc::now();
    // Ne garder que les sanctions encore actives (pas expirees)
    ...
    // Trier par proximite d'expiration (les plus urgentes en premier)
