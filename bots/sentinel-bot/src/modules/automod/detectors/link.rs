...
mod tests {
    ...
    #[test]
    fn https_simple() { assert!(detect("https://example.com", false, &[])); }
    #[test]
    fn http_simple() { assert!(detect("http://example.com", false, &[])); }
    #[test]
    fn https_with_path() { assert!(detect("https://example.com/page/sub?q=1", false, &[])); }
    #[test]
    fn url_in_text() { assert!(detect("Va voir https://example.com pour plus d'infos", false, &[])); }
    #[test]
    fn multiple_urls() { assert!(detect("https://a.com et https://b.com", false, &[])); }

    // ── Discord invites ──

    #[test]
    fn discord_gg_blocked_by_default() { assert!(detect("Rejoins discord.gg/abc123", false, &[])); }
    #[test]
    fn discord_com_invite_blocked_by_default() { assert!(detect("discord.com/invite/test", false, &[])); }
    ...
    fn multiple_urls_one_allowed_one_not() {
        ...
        assert!(detect("https://twitch.tv/ok et https://badsite.com/hack", false, &allowed));
    }
    #[test]
    fn all_urls_allowed() {
        let allowed = vec!["twitch.tv".to_string(), "youtube.com".to_string()];
        assert!(!detect("https://twitch.tv/ok https://youtube.com/watch?v=abc", false, &allowed));
    }

    // ── Pas de lien ──

    #[test]
    fn no_protocol() { assert!(!detect("Mon site est example.com", false, &[])); }
    #[test]
    fn clean_text() { assert!(!detect("Salut tout le monde", false, &[])); }
    #[test]
    fn empty() { assert!(!detect("", false, &[])); }
    #[test]
    fn email_not_url() { assert!(!detect("contact@example.com", false, &[])); }
    #[test]
    fn dotted_words() { assert!(!detect("e.g. c'est a dire", false, &[])); }
    #[test]
    fn ip_without_protocol() { assert!(!detect("192.168.1.1", false, &[])); }
    #[test]
    fn ip_with_protocol() { assert!(detect("http://192.168.1.1/admin", false, &[])); }
}
