...
//!   Discord vers les sous-handlers `super::handler_impls::*`),
...
use super::api_client::{ApiClient, AuditEvent};
use super::commands;
use super::handler_impls;

mod type_keys;
mod watched_users;

// Re-exports pour préserver `super::handler_impl::{WeeklyTrackerKey, ...}`
    ...
    async fn message(&self, ctx: Context, msg: Message) {
        ...
        if let Some(cache) = data.get::<MessageCacheKey>() {
            cache.store(
                guild_id,
                msg.id,
                super::message_cache::CachedMessage {
