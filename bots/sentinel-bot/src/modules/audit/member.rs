...
use super::handler_impl::{AnomalyDetectorKey, Handler, WeeklyTrackerKey};
...
pub async fn handle_update(
    ...
    if old_nick != new_nick {
        ...
        let embed = info_embed("✏️ Pseudo modifie")
            .field("Membre", format!("<@{}>", new_member.user.id), true)
            ...
            .timestamp(serenity::model::Timestamp::now())
            .footer(serenity::builder::CreateEmbedFooter::new("Audit | Sentinel"));
        Handler::post_to_channel(ctx, &gid_str, &["profile_edit_channel_id"], embed).await;
    ...
    if old_avatar != new_avatar {
        ...
        // Embed Discord → profile_edit_channel_id
        let mut avatar_embed = info_embed("🖼️ Avatar serveur modifie")
            .field("Membre", format!("<@{}>", new_member.user.id), true)
            .field("ID", user_id.clone(), true)
            .timestamp(serenity::model::Timestamp::now())
            .footer(serenity::builder::CreateEmbedFooter::new("Audit | Sentinel"));
        ...
        Handler::post_to_channel(ctx, &gid_str, &["profile_edit_channel_id"], avatar_embed).await;
