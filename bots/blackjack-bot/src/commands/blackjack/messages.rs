//! Phrases "fun" piochées aléatoirement dans les embeds de fin de partie.
//!
//! Les placeholders `{joueur}`, `{total}`, `{croupier}`, `{gain}`, `{mise}`
//! sont remplacés par l'appelant (`embeds::build_game_embed`).
...
/// Pioche une phrase aléatoire dans un pool.
