...
mod tests {
    ...
    fn parse_ignores_invalid() {
        ...
        let faq = parse_faq(raw);
        assert_eq!(faq.len(), 1);
        assert_eq!(faq[0].question, "Q");
    }

    #[test]
    fn parse_trims() {
        let raw = "  Question  |  Reponse  ";
        let faq = parse_faq(raw);
        assert_eq!(faq[0].question, "Question");
        assert_eq!(faq[0].answer, "Reponse");
    }
    ...
    #[test]
    fn parse_answer_with_pipe() {
        let raw = "Q|A with | pipe";
        let faq = parse_faq(raw);
        assert_eq!(faq[0].answer, "A with | pipe");
    }
}
