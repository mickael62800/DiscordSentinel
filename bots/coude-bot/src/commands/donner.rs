...
pub async fn handle(ctx: &Context, command: &CommandInteraction) {
    ...
    if don_type == "coins" {
        ...
        // Debit donor
        if let Err(e) = api
            .update_player_coins(&guild_id, &donor_id, -amount)
            .await
        {
            reply_ephemeral(ctx, command, &format!("Erreur API : {e}")).await;
            return;
        ...
        // Credit receiver (after tax)
        if let Err(e) = api
            .update_player_coins(&guild_id, &target_id_str, received)
            .await
        {
            // Try to refund donor on failure
            let _ = api
                .update_player_coins(&guild_id, &donor_id, amount)
                .await;
            reply_ephemeral(ctx, command, &format!("Erreur API : {e}")).await;
        ...
        // Transfer items one by one (check + remove + give)
        for i in 0..qty {
            ...
            if !has {
                if i == 0 {
                    reply_ephemeral(
                        ...
                        command,
                        &format!(
                            "Tu n'as pas de **{}** dans ton inventaire !",
                            item.name
                        ),
                    )
                    .await;
            ...
            if let Err(e) = api.add_item(&guild_id, &target_id_str, item_key).await {
                // Try to return the item to donor on failure
