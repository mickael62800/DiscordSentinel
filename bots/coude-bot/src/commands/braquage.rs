...
pub async fn handle(ctx: &Context, command: &CommandInteraction) {
    ...
    match api.attempt_heist(&guild_id, &user_id).await {
        ...
        Err(e) => {
            ...
            // - cooldown non ecoule
            // - caisse vide
            crate::interaction_helper::followup_text(ctx, command, &format!("\u{1f6ab} {e}")).await;
        }
    }
}

fn build_result_embed(user_id: &str, r: &crate::api_client::HeistResult) -> CreateEmbed {
    if r.success {
        ...
        CreateEmbed::new()
            ...
            .description(format!(
                "\u{1f3ad} <@{}> a reussi son coup sur la caisse communautaire !\n\n\
        ...
        CreateEmbed::new()
            ...
            .description(format!(
                "\u{1f46e} <@{}> s'est fait choper en flagrant delit !\n\n\
                 \u{1f3b2} Chance : **{} %**\n\
                 \u{1f6e0}\u{fe0f} {}\
                 {}",
                user_id, r.chance_percent, tools_line, prison_msg
            ))
            .color(0xE74C3C)
            .footer(CreateEmbedFooter::new(
