...
//!
//! Le premier membre present dans le vocal qui clique devient le nouveau
//! owner. Le bouton est desactive apres la premiere candidature acceptee.
...
use crate::handler::{TextToVoiceMapKey, VoiceOwnerMapKey};
...
pub async fn handle(ctx: &Context, component: &ComponentInteraction) {
    ...
    // Verifier que le claimant est dans le vocal.
    ...
    // Appel API pour persister le transfert.
    {
    ...
    // Permissions Discord — donner admin au nouveau owner.
    ...
    // Donner acces au panel admin texte.
    let text_channel_id = {
        let data = ctx.data.read().await;
    ...
    // Desactiver le bouton en editant le message original.
