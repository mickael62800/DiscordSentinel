...
async fn strike_config_defaults() {
    ...
    let row = sqlx::query_as::<_, (i64, bool)>(
        "SELECT window_secs, enabled FROM strike_config WHERE guild_id = $1",
    )
    .bind(&gid)
    .fetch_one(&p)
    .await
...
async fn strike_config_custom_thresholds() {
    ...
    sqlx::query(
        "INSERT INTO strike_config (guild_id, thresholds, window_secs) VALUES ($1, $2, 7200)",
    )
    .bind(&gid)
    .bind(&thresholds)
    .execute(&p)
    ...
    let row = sqlx::query_as::<_, (i64, serde_json::Value)>(
        "SELECT window_secs, thresholds FROM strike_config WHERE guild_id = $1",
    )
    .bind(&gid)
    .fetch_one(&p)
    .await
...
async fn level_config_custom() {
    ...
    let msg = sqlx::query_as::<_, (String,)>(
        "SELECT level_up_message FROM level_config WHERE guild_id = $1",
    )
    .bind(&gid)
    .fetch_one(&p)
    .await
