...
async fn save_and_find_by_guild() {
    ...
    let g = fresh_id();
    let u = fresh_id();
    repo.save(&sample_infraction(&g, &u, Action::Warn, 3.0))
        .await
        .unwrap();
    let list = repo
        .find_by_guild(
...
async fn find_by_guild_filter_by_user_id() {
    ...
    let u1 = fresh_id();
    let u2 = fresh_id();
    repo.save(&sample_infraction(&g, &u1, Action::Warn, 2.0))
        .await
        .unwrap();
    repo.save(&sample_infraction(&g, &u2, Action::Mute, 5.0))
        .await
        .unwrap();
    let list = repo
        .find_by_guild(
...
async fn find_by_guild_filter_by_action() {
    ...
    let g = fresh_id();
    let u = fresh_id();
    repo.save(&sample_infraction(&g, &u, Action::Warn, 2.0))
        .await
        .unwrap();
    repo.save(&sample_infraction(&g, &u, Action::Mute, 5.0))
        .await
        .unwrap();
    repo.save(&sample_infraction(&g, &u, Action::Ban, 8.0))
        .await
        .unwrap();
    let mutes = repo
        .find_by_guild(
...
async fn find_by_guild_respects_limit_and_offset() {
    ...
    let g = fresh_id();
    for _ in 0..5 {
        repo.save(&sample_infraction(&g, &fresh_id(), Action::Warn, 1.0))
            .await
            .unwrap();
    }
    let page1 = repo
...
async fn find_all_paginates_across_guilds() {
    ...
    let g1 = fresh_id();
    let g2 = fresh_id();
    repo.save(&sample_infraction(&g1, &fresh_id(), Action::Warn, 1.0))
        .await
        .unwrap();
    repo.save(&sample_infraction(&g2, &fresh_id(), Action::Mute, 5.0))
        .await
        .unwrap();
...
async fn count_today_returns_recent_count() {
    ...
    repo.save(&sample_infraction(&g, &fresh_id(), Action::Warn, 1.0))
        .await
        .unwrap();
    repo.save(&sample_infraction(&g, &fresh_id(), Action::Mute, 3.0))
        .await
        .unwrap();
...
async fn delete_older_than_days_scopes_to_guild() {
    ...
    repo.save(&sample_infraction(&g1, &fresh_id(), Action::Warn, 1.0))
        .await
        .unwrap();
    repo.save(&sample_infraction(&g2, &fresh_id(), Action::Mute, 3.0))
        .await
        .unwrap();
    ...
    assert!(
        !list_g2.is_empty(),
        "g2 infractions ne devraient pas etre touchees"
    );
}
