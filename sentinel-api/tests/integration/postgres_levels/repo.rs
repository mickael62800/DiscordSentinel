...
fn sample_config(guild: &str) -> LevelConfig {
    ...
    LevelConfig {
        guild_id: guild.into(),
        xp_per_message: 15, xp_per_voice_minute: 2, xp_cooldown_secs: 60,
...
fn sample_user_level(guild: &str, user: &str) -> UserLevel {
    ...
    UserLevel {
        ...
        username: user.into(),
        xp: 500, level: 2, xp_text: 400, level_text: 2,
...
async fn user_level_none_when_absent() {
    let repo = PgLevelRepository::new(pool().await);
    assert!(repo.get_user_level(&fresh_id(), &fresh_id()).await.unwrap().is_none());
}
...
async fn user_level_upsert_and_get() {
    ...
    repo.upsert_user_level(&sample_user_level(&g, &u)).await.unwrap();
...
async fn add_xp_atomic_creates_or_accumulates() {
    let repo = PgLevelRepository::new(pool().await);
    let g = fresh_id(); let u = fresh_id();
    let first = repo.add_xp_atomic(&g, &u, "Alice", 50, XpSource::Text).await.unwrap();
    assert_eq!(first.xp_text, 50);
    assert!(first.xp >= 50);
    let second = repo.add_xp_atomic(&g, &u, "Alice", 30, XpSource::Voice).await.unwrap();
...
async fn leaderboard_by_source_text_uses_xp_text_column() {
    ...
    let user = fresh_id();
    let mut ul = sample_user_level(&g, &user);
    ul.xp_text = 300; ul.xp_voice = 50; ul.xp = 350;
    repo.upsert_user_level(&ul).await.unwrap();
    let by_text = repo.get_leaderboard_by_source(&g, XpSource::Text, 5).await.unwrap();
