...
impl MockGameRepo {
    ...
    fn with_game(self, g: Game) -> Self {
        self.games.lock().unwrap().push(g);
        self
    }
}
    ...
    async fn update(
        ...
        match g {
            Some(g) => {
                ...
                if let Some(e) = emoji {
                    g.emoji = e.map(str::to_string);
                }
                if let Some(c) = category {
                    g.category = c.map(str::to_string);
                }
                Ok(Some(g.clone()))
            }
            None => Ok(None),
...
async fn update_game_rename_and_category() {
    ...
    let (status, json) =
        patch_json(app, &format!("/api/games/111111111111111111/{id}"), body).await;
    assert_eq!(status, StatusCode::OK);
...
async fn update_game_not_found_returns_404() {
    ...
    let (status, _) = patch_json(
        app,
        &format!("/api/games/111111111111111111/{fake_id}"),
        body,
    )
    .await;
...
async fn update_game_with_rbac_viewer_forbidden() {
    ...
    let req = test_helpers::request_with_rbac(
        "PATCH",
        &format!("/api/games/111111111111111111/{id}"),
        "555555555555555555",
        Some(Role::Viewer),
...
async fn set_role_id_with_value() {
    ...
    let (status, json) = patch_json(
        app,
        &format!("/api/games/111111111111111111/{id}/role"),
        body,
    )
    .await;
    assert_eq!(status, StatusCode::OK);
...
async fn get_game_by_name_found() {
    let repo =
        Arc::new(MockGameRepo::new().with_game(sample_game("111111111111111111", "Valorant")));
    let app = build_app(repo);
...
async fn list_by_category_filter() {
    ...
    let (status, json) = get(
        app,
        "/api/games/111111111111111111/by-category?category=fps",
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    let arr = json.as_array().unwrap();
...
async fn find_panel_by_message_not_found_returns_null() {
    ...
    let (status, json) = get(
        app,
        "/api/games/111111111111111111/panels/by-message/m-unknown",
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(json, serde_json::Value::Null);
...
async fn upload_emoji_success_with_mock_discord() {
    ...
    let (status, json) =
        post_multipart(app, "/api/games/111111111111111111/upload-emoji", ct, body).await;
    assert_eq!(status, StatusCode::OK);
...
async fn upload_emoji_missing_name_422() {
    let app = build_app(Arc::new(MockGameRepo::new()));
    let (ct, body) = multipart_body(&[("image", "x.png", Some("image/png"), b"xx")]);
    let (status, json) =
        post_multipart(app, "/api/games/111111111111111111/upload-emoji", ct, body).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
...
async fn upload_emoji_missing_image_422() {
    let app = build_app(Arc::new(MockGameRepo::new()));
    let (ct, body) = multipart_body(&[("name", "n", None, &[])]);
    let (status, json) =
        post_multipart(app, "/api/games/111111111111111111/upload-emoji", ct, body).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
