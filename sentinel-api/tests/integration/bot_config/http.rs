...
impl MockBotConfigRepo {
    ...
    fn with_def(self, d: BotDefinition) -> Self {
        self.definitions.lock().unwrap().push(d);
        self
    }
    fn with_cfg(self, c: BotGuildConfig) -> Self {
        self.configs.lock().unwrap().push(c);
        self
    }
}
...
async fn set_config_upserts_on_same_key() {
    let repo = Arc::new(MockBotConfigRepo::new().with_cfg(sample_cfg(
        "111111111111111111",
        "b",
        "k",
        "old",
...
async fn get_guild_config_second_call_hits_cache() {
    ...
    let repo =
        Arc::new(MockBotConfigRepo::new().with_cfg(sample_cfg(&guild_id, "bot-x", "k", "v")));
    let app1 = build_app(repo.clone());
...
async fn get_bot_config_second_call_hits_cache() {
    ...
    let repo =
        Arc::new(MockBotConfigRepo::new().with_cfg(sample_cfg(&guild_id, "bot-y", "k", "v")));
    let app1 = build_app(repo.clone());
