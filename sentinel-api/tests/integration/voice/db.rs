...
async fn voice_channel_close_sets_status() {
    ...
    .unwrap();
    let status =
        sqlx::query_as::<_, (String,)>("SELECT channel_status FROM voice_channels WHERE id = $1")
            .bind(id)
            .fetch_one(&p)
            .await
            .unwrap()
...
async fn voice_channel_co_admins() {
    ...
    let count = sqlx::query_as::<_, (i64,)>(
        "SELECT COUNT(*) FROM voice_channel_co_admins WHERE voice_channel_id = $1",
    )
    .bind(vc_id)
    .fetch_one(&p)
    .await
...
async fn voice_channel_bans_cascade() {
    ...
    sqlx::query("DELETE FROM voice_channels WHERE id = $1")
        ...
        .await
        .unwrap();
    let bans = sqlx::query_as::<_, (i64,)>(
        "SELECT COUNT(*) FROM voice_channel_bans WHERE voice_channel_id = $1",
    )
    .bind(vc_id)
    .fetch_one(&p)
    .await
...
async fn voice_channel_lock_toggle() {
    ...
    sqlx::query("UPDATE voice_channels SET locked = true WHERE id = $1")
        .bind(id)
        .execute(&p)
        .await
        .unwrap();
    ...
    sqlx::query("UPDATE voice_channels SET locked = false WHERE id = $1")
        .bind(id)
        .execute(&p)
        .await
        .unwrap();
