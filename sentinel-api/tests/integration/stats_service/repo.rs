...
async fn get_user_stats_none_for_unknown_user() {
    let svc = build().await;
    assert!(svc
        .get_user_stats(&fresh_id(), &fresh_id())
        .await
        .unwrap()
        .is_none());
}
...
async fn get_dashboard_stats_with_redis_available() {
    ...
    let _: () = conn
        .sadd::<_, _, ()>("bots:known", &bot_online)
        .await
        .unwrap();
    let _: () = conn
        .sadd::<_, _, ()>("bots:known", &bot_offline)
        .await
        .unwrap();
    let _: () = conn
        .sadd::<_, _, ()>("bots:known", &worker_online)
        .await
        .unwrap();
    let _: () = conn
        .sadd::<_, _, ()>("bots:known", &worker_offline)
        .await
        .unwrap();
    let _: () = conn
        .set_ex::<_, _, ()>(format!("bot:online:{bot_online}"), "1", 60)
        .await
        .unwrap();
    let _: () = conn
        .set_ex::<_, _, ()>(format!("bot:online:{worker_online}"), "1", 60)
        .await
        .unwrap();
    // Pour couvrir la branche 'ping_test' du health check, on seed la cle.
    let _: () = conn
        .set_ex::<_, _, ()>("ping_test", "pong", 60)
        .await
        .unwrap();
    ...
    // Cleanup
    let _: () = conn
        .srem::<_, _, ()>("bots:known", &bot_online)
        .await
        .unwrap();
    let _: () = conn
        .srem::<_, _, ()>("bots:known", &bot_offline)
        .await
        .unwrap();
    let _: () = conn
        .srem::<_, _, ()>("bots:known", &worker_online)
        .await
        .unwrap();
    let _: () = conn
        .srem::<_, _, ()>("bots:known", &worker_offline)
        .await
        .unwrap();
    let _: () = conn
        .del::<_, ()>(format!("bot:online:{bot_online}"))
        .await
        .unwrap();
    let _: () = conn
        .del::<_, ()>(format!("bot:online:{worker_online}"))
        .await
        .unwrap();
}
