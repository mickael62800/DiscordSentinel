...
async fn auto_role_add_and_list() {
    ...
    let gid = ugid();
    sqlx::query(
        "INSERT INTO auto_roles (guild_id, role_id, role_name) VALUES ($1, '111', 'Member')",
    )
    .bind(&gid)
    .execute(&p)
    .await
...
async fn auto_role_unique_per_guild() {
    ...
    sqlx::query("INSERT INTO auto_roles (guild_id, role_id, role_name) VALUES ($1, '111', 'A')")
        ...
        .unwrap();
    let dup = sqlx::query(
        "INSERT INTO auto_roles (guild_id, role_id, role_name) VALUES ($1, '111', 'B')",
    )
    .bind(&gid)
    .execute(&p)
    .await;
...
async fn role_panel_entries_cascade_delete() {
    ...
    sqlx::query(
        "INSERT INTO role_panel_entries (panel_id, role_id, label) VALUES ($1, '111', 'R')",
    )
    .bind(panel_id)
    .execute(&p)
    .await
    ...
    sqlx::query("DELETE FROM role_panels WHERE id = $1")
        ...
        .unwrap();
    let count =
        sqlx::query_as::<_, (i64,)>("SELECT COUNT(*) FROM role_panel_entries WHERE panel_id = $1")
            .bind(panel_id)
            .fetch_one(&p)
            .await
            .unwrap()
...
async fn sponsorship_create() {
    ...
    let gid = ugid();
    sqlx::query(
        "INSERT INTO sponsorships (guild_id, sponsor_id, sponsored_id) VALUES ($1, '111', '222')",
    )
    .bind(&gid)
    .execute(&p)
    .await
    .unwrap();
    let count =
        sqlx::query_as::<_, (i64,)>("SELECT COUNT(*) FROM sponsorships WHERE guild_id = $1")
            .bind(&gid)
            .fetch_one(&p)
            .await
            .unwrap()
...
async fn sponsorship_unique_per_sponsored() {
    ...
    .unwrap();
    let dup = sqlx::query(
        "INSERT INTO sponsorships (guild_id, sponsor_id, sponsored_id) VALUES ($1, '333', '222')",
    )
    .bind(&gid)
    .execute(&p)
    .await;
...
async fn sponsorship_count_per_sponsor() {
    ...
    for sponsored in &["a", "b", "c"] {
        sqlx::query(
            "INSERT INTO sponsorships (guild_id, sponsor_id, sponsored_id) VALUES ($1, '111', $2)",
        )
        .bind(&gid)
        .bind(sponsored)
        .execute(&p)
...
mod community_grpc {
    ...
    async fn create_sponsorship_and_list_round_trip() {
        ...
        let resp = g
            .list_sponsorships(Request::new(proto::ListSponsorshipsRequest {
                guild_id: gid.clone().into(),
            }))
            .await
            .unwrap();
    ...
    async fn create_sponsorship_duplicate_is_noop_via_on_conflict() {
        ...
        let result = g
            .create_sponsorship(Request::new(proto::CreateSponsorshipRequest {
                guild_id: gid.clone().into(),
                sponsor_id: ugid(),
                sponsored_id: sponsored.clone(),
            }))
    ...
    async fn create_temp_role_rejects_bad_rfc3339() {
        ...
        let g = grpc(p);
        let err = g
            .create_temp_role(Request::new(proto::CreateTempRoleRequest {
                guild_id: ugid(),
                user_id: ugid(),
                role_id: ugid(),
    ...
    async fn create_temp_role_and_list_only_active() {
        ...
        let list = g
            .list_temp_roles(Request::new(proto::ListTempRolesRequest {
                guild_id: gid.clone().into(),
            }))
            .await
            .unwrap()
    ...
    async fn create_temp_role_upsert_on_conflict() {
        ...
        let list = g
            .list_temp_roles(Request::new(proto::ListTempRolesRequest { guild_id: gid }))
            .await
            .unwrap()
            .into_inner()
            .roles;
