...
fn build_test_state_moderation(
...
async fn get_history_empty() {
    ...
    let (status, json) = get(
        app,
        "/api/moderation/history/111111111111111111/444444444444444444",
    )
    .await;
    assert_eq!(status, StatusCode::OK);
...
async fn get_history_different_guild_returns_empty() {
    let uc = MockModerationUC::new().with_action(make_action(
        Uuid::new_v4(),
        "111111111111111111",
        "444444444444444444",
        "Alice",
    ...
    let (status, json) = get(
        app,
        "/api/moderation/history/222222222222222222/444444444444444444",
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(json["total_warns"], 0);
...
async fn list_bans_returns_only_bans() {
    ...
    assert!(bans
        .iter()
        .all(|b| b["action_type"].as_str().unwrap().starts_with("ban")));
}
