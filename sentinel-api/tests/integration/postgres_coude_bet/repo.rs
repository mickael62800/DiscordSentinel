...
async fn seed_combat(p: &PgPool, g: &str, att: &str, def: &str, status: &str) -> Uuid {
    ...
    ).bind(id).bind(g).bind(att).bind(def).bind(status).execute(p).await.unwrap();
    id
}
...
async fn list_for_combat_empty() {
    ...
    assert!(repo.list_for_combat(Uuid::new_v4()).await.unwrap().is_empty());
}
...
async fn place_debits_wallet_and_inserts_bet() {
    ...
    let (coins,): (i64,) = sqlx::query_as("SELECT coins FROM user_wallets WHERE guild_id = $1 AND user_id = $2")
        .bind(&g).bind(&bettor).fetch_one(&p).await.unwrap();
    assert_eq!(coins, 700);
}
...
async fn place_multiple_bets_on_same_combat() {
    ...
    let att = fresh_id(); let def = fresh_id();
    let combat_id = seed_combat(&p, &g, &att, &def, "betting").await;
    let b1 = fresh_id(); let b2 = fresh_id();
...
async fn refund_unresolved_credits_back_bettors() {
    ...
    let (coins,): (i64,) = sqlx::query_as("SELECT coins FROM user_wallets WHERE guild_id = $1 AND user_id = $2")
        .bind(&g).bind(&b1).fetch_one(&p).await.unwrap();
...
async fn apply_resolution_winner_gets_payout_draw_refunds() {
    ...
    let (b1_coins,): (i64,) = sqlx::query_as("SELECT coins FROM user_wallets WHERE guild_id = $1 AND user_id = $2")
        .bind(&g).bind(&b1).fetch_one(&p).await.unwrap();
    let (b2_coins,): (i64,) = sqlx::query_as("SELECT coins FROM user_wallets WHERE guild_id = $1 AND user_id = $2")
        .bind(&g).bind(&b2).fetch_one(&p).await.unwrap();
    // b1 : 1000 - 200 + payout. payout > 0 si quelqu'un a backe le perdant.
    assert!(b1_coins > 800, "b1 devrait recuperer + payout, got {b1_coins}");
