...
async fn deposit_increases_balance_and_total_collected() {
    let repo = PgCashboxRepository::new(pool().await);
    let g = fresh_id();
    repo.deposit(&g, 100, CashboxSource::ShopPurchase)
        .await
        .unwrap();
    repo.deposit(&g, 200, CashboxSource::BetCommission)
        .await
        .unwrap();
    let cb = repo.get_or_create(&g).await.unwrap();
...
async fn deposit_creates_cashbox_if_absent() {
    ...
    repo.deposit(&g, 50, CashboxSource::DonationTax)
        .await
        .unwrap();
    let cb = repo.get_or_create(&g).await.unwrap();
...
async fn withdraw_partial_amount() {
    let repo = PgCashboxRepository::new(pool().await);
    let g = fresh_id();
    repo.deposit(&g, 500, CashboxSource::ShopPurchase)
        .await
        .unwrap();
...
async fn claim_all_for_redistribution_empties_cashbox() {
    let repo = PgCashboxRepository::new(pool().await);
    let g = fresh_id();
    repo.deposit(&g, 750, CashboxSource::BetCommission)
        .await
        .unwrap();
...
async fn list_redistributions_empty_when_none() {
    let repo = PgCashboxRepository::new(pool().await);
    assert!(repo
        .list_redistributions(&fresh_id(), 10)
        .await
        .unwrap()
        .is_empty());
}
...
async fn list_active_players_empty_when_no_combat() {
    let repo = PgCashboxRepository::new(pool().await);
    assert!(repo
        .list_active_players(&fresh_id(), 7)
        .await
        .unwrap()
        .is_empty());
}
