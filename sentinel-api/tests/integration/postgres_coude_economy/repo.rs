...
async fn record_steal_stats_updates_both_players() {
    ...
    repo.record_steal_stats(&g, &thief, &victim, 150)
        .await
        .unwrap();
    ...
    let (lost,): (i64,) =
        sqlx::query_as("SELECT total_lost FROM coude_players WHERE guild_id = $1 AND user_id = $2")
            .bind(&g)
            .bind(&victim)
            .fetch_one(&p)
            .await
...
async fn record_casino_faillite_returns_total_lost() {
    ...
    let total = repo
        .record_casino_faillite_stats(&g, &u, 500)
        .await
        .unwrap();
...
async fn count_casino_today_zero_for_fresh_user() {
    let repo = PgEconomyRepository::new(pool().await);
    assert_eq!(
        repo.count_casino_today(&fresh_id(), &fresh_id())
            .await
            .unwrap(),
        0
    );
...
async fn count_steal_today_zero_for_fresh_user() {
    let repo = PgEconomyRepository::new(pool().await);
    assert_eq!(
        repo.count_steal_today(&fresh_id(), &fresh_id())
            .await
            .unwrap(),
        0
    );
