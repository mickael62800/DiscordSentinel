...
use sentinel_core::domain::entities::game::quota::GuildQuotaState;
use sentinel_core::domain::entities::game::server::{
    ...
    fn build_spec(
        ...
        if let Some(host_port) = server.host_port {
            ...
            let proto = match template.port_protocol {
                sentinel_core::domain::entities::game::template::PortProtocol::Tcp => PortProtocol::Tcp,
                sentinel_core::domain::entities::game::template::PortProtocol::Udp => PortProtocol::Udp,
            };
...
impl ManageGameServersUseCase for ManageGameServersService {
    ...
    async fn update_config(
        ...
        for k in entries.keys() {
            sentinel_core::domain::entities::game::config::validate_config_key(k)
                .map_err(DomainError::ValidationError)?;
        }
        self.config_repo
