...
impl TauntsRepository for MockTauntsRepo {
    ...
    async fn set_channel(&self, _: &str, c: Option<&str>) -> Result<(), DomainError> {
        self.set_channel_calls.lock().unwrap().push(c.map(String::from));
        Ok(())
    }
    async fn set_enabled(&self, _: &str, e: bool) -> Result<(), DomainError> {
        self.set_enabled_calls.lock().unwrap().push(e); Ok(())
    }
    async fn set_rename_enabled(&self, _: &str, e: bool) -> Result<(), DomainError> {
        self.set_rename_calls.lock().unwrap().push(e); Ok(())
    }
    async fn set_messages_enabled(&self, _: &str, e: bool) -> Result<(), DomainError> {
        self.set_messages_calls.lock().unwrap().push(e); Ok(())
    }
...
impl PlayerRepository for MockPlayerRepo {
    ...
    async fn record_loss(&self, _: &str, _: &str, _: i64) -> Result<bool, DomainError> { unimplemented!() }
    async fn record_draw(&self, _: &str, _: &str, _: i64) -> Result<bool, DomainError> { unimplemented!() }
    ...
    async fn reset_combat_streaks(&self, _: &str, _: &str) -> Result<(), DomainError> {
        *self.reset_combat_calls.lock().unwrap() += 1; Ok(())
    }
    async fn touch_steal_victim_streak(&self, _: &str, _: &str) -> Result<Option<i32>, DomainError> {
        Ok(*self.steal_streak.lock().unwrap())
    }
    async fn reset_steal_victim_streak(&self, _: &str, _: &str) -> Result<(), DomainError> {
        *self.reset_steal_calls.lock().unwrap() += 1; Ok(())
    }
    ...
    async fn increment_cowardice(&self, _: &str, _: &str) -> Result<Option<i32>, DomainError> { unimplemented!() }
    async fn increment_chaos(&self, _: &str, _: &str) -> Result<bool, DomainError> { unimplemented!() }
...
impl BotConfigRepository for MockBotConfigRepo {
    async fn get_definitions(&self) -> Result<Vec<sentinel_core::domain::entities::system::bot_config::BotDefinition>, DomainError> { Ok(vec![]) }
...
async fn on_player_won_no_threshold_returns_none() {
    ...
    *p.win_streak.lock().unwrap() = Some(1); // sous threshold (first is 3)
    let svc = make_service(Arc::new(MockTauntsRepo::new()), p,
...
async fn on_player_lost_and_drew() {
    ...
    *p.loss_streak.lock().unwrap() = Some(3);
    let svc = make_service(Arc::new(MockTauntsRepo::new()), p.clone(),
        Arc::new(MockBotConfigRepo::default()));
...
async fn bankruptcy_disabled_by_config_returns_none() {
    let bot = Arc::new(MockBotConfigRepo::with(&[("bankruptcy_taunt_enabled", "false")]));
    let svc = make_service(Arc::new(MockTauntsRepo::new()),
        Arc::new(MockPlayerRepo::default()), bot);
...
async fn donor_below_threshold_returns_none() {
    let svc = make_service(Arc::new(MockTauntsRepo::new()),
        ...
        Arc::new(MockBotConfigRepo::default()));
    // default = 1_000
    assert!(svc.on_generous_donor("g", "u", 500).await.unwrap().is_none());
}
...
async fn donor_at_threshold_emits() {
    let svc = make_service(Arc::new(MockTauntsRepo::new()),
        Arc::new(MockPlayerRepo::default()),
        Arc::new(MockBotConfigRepo::default()));
    assert!(svc.on_generous_donor("g", "u", 1_000).await.unwrap().is_some());
}

#[tokio::test]
async fn config_setters_forward_to_repo() {
    let t = Arc::new(MockTauntsRepo::new());
    let svc = make_service(t.clone(),
        Arc::new(MockPlayerRepo::default()),
        Arc::new(MockBotConfigRepo::default()));
    ...
    assert_eq!(t.set_channel_calls.lock().unwrap()[0].as_deref(), Some("c2"));
