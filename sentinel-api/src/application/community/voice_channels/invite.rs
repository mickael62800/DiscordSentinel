...
impl ManageVoiceChannelsService {
    ...
    pub(super) async fn create_invite_link_impl(&self, cmd: CreateInviteLinkCommand) -> Result<VoiceChannelInviteLink, DomainError> {
    ...
    pub(super) async fn list_invite_links_impl(&self, channel_id: &str) -> Result<Vec<VoiceChannelInviteLink>, DomainError> {
        let channel = self.resolve_channel(channel_id).await?;
        self.repo.find_invite_links(channel.id).await
    }

    pub(super) async fn use_invite_link_impl(&self, cmd: UseInviteLinkCommand) -> Result<VoiceChannelInviteLink, DomainError> {
    ...
    pub(super) async fn revoke_invite_link_impl(&self, channel_id: &str, link_id: &str) -> Result<(), DomainError> {
        let channel = self.resolve_channel(channel_id).await?;
