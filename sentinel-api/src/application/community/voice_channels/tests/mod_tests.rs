    ...
    impl BotConfigRepository for MockBotConfig {
        ...
        async fn get_config(&self, _: &str, _: &str) -> Result<Vec<sentinel_core::domain::entities::system::bot_config::BotGuildConfig>, DomainError> { Ok(vec![]) }
        async fn get_all_config(&self, _: &str) -> Result<Vec<sentinel_core::domain::entities::system::bot_config::BotGuildConfig>, DomainError> { Ok(vec![]) }
