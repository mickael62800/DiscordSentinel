...
pub async fn create_table(
    ...
    let mut shoe: Vec<sentinel_core::domain::entities::casino::blackjack::Card> = Vec::with_capacity(BLACKJACK_SHOE_TOTAL_CARDS);
    ...
    let table = state.blackjack_table_repo
        .create(&dto.guild_id, &dto.channel_id, &dto.owner_id, &dto.owner_name, &shoe_json)
        .await?;
    Ok(Json(table))
}
...
pub async fn join_table(
    ...
    let info = state.blackjack_table_repo.get_status_and_guild(&table_id).await?;
    match &info {
        Some((s, _)) if s == "open" => {}
    ...
    let max_players: i64 = state.bot_config_repo
        ...
        .unwrap_or(sentinel_core::domain::entities::casino::blackjack::DEFAULT_BLACKJACK_MAX_PLAYERS);
...
pub async fn get_table_by_channel(
    ...
    let table = state.blackjack_table_repo.find_open_by_channel(&channel_id).await?;
    Ok(Json(table))
}
