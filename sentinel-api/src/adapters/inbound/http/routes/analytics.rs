...
pub fn inner() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/actions",
            get(handlers::audit::analytics::get_action_distribution),
        )
        .route(
            "/top-infractors",
            get(handlers::audit::analytics::get_top_infractors),
        )
        .route(
            "/moderation-trend",
            get(handlers::audit::analytics::get_moderation_trend),
        )
        .route(
            "/peak-hours",
            get(handlers::audit::analytics::get_peak_hours),
        )
