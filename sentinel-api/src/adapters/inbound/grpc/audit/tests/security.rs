use super::*;
    ...
    impl ManageSecurityUseCase for MockSecurityUc {
        ...
        async fn list_events(&self, guild_id: Option<&str>) -> Result<Vec<SecurityEvent>, DomainError> {
            self.list_calls.lock().unwrap().push(guild_id.map(String::from));
    ...
    fn grpc(uc: Arc<MockSecurityUc>) -> SecurityGrpc { SecurityGrpc { uc } }

    #[tokio::test]
    async fn report_event_returns_persisted_event() {
        let uc = Arc::new(MockSecurityUc::default());
        let g = grpc(uc.clone());
        let resp = g.report_event(Request::new(proto::ReportEventRequest {
            guild_id: "g1".into(),
            event_type: "raid".into(),
            severity: "critical".into(),
    ...
    async fn list_events_with_guild_id_filter() {
        let uc = Arc::new(MockSecurityUc::default());
        let g = grpc(uc.clone());
        let _ = g.list_events(Request::new(proto::ListEventsRequest {
    ...
    async fn list_events_none_filter_returns_all_guilds() {
        ...
        let g = grpc(uc.clone());
        let resp = g.list_events(Request::new(proto::ListEventsRequest {
            guild_id: None,
        })).await.unwrap();
    ...
    async fn analyze_new_member_delegates_and_maps_decision() {
        ...
        *uc.analyze_returns.lock().unwrap() = SecurityDecision {
        ...
        let g = grpc(uc);
        let resp = g.analyze_new_member(Request::new(proto::AnalyzeNewMemberRequest {
            guild_id: "g".into(),
            user_id: "u".into(),
    ...
    async fn analyze_new_member_maps_recent_joins_to_domain() {
        ...
        let _ = g.analyze_new_member(Request::new(proto::AnalyzeNewMemberRequest {
            guild_id: "g".into(),
            user_id: "u".into(),
            username: "x".into(),
            has_avatar: true,
