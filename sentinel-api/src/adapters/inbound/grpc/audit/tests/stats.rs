use super::*;
    ...
    impl ManageStatsUseCase for MockStatsUc {
        ...
        async fn get_guild_overview(&self, g: &str) -> Result<GuildStatsOverview, DomainError> {
            Ok(self.overview_return.lock().unwrap().clone().unwrap_or(GuildStatsOverview {
                guild_id: g.into(),
                total_messages: 0, total_voice_seconds: 0, active_members: 0,
                total_infractions: 0, total_warns: 0, total_mutes: 0, total_bans: 0,
                top_members: vec![],
    ...
    fn sample_user() -> UserStats {
        UserStats {
            ...
            message_count: 100, voice_seconds: 3600,
            updated_at: Utc::now(),
        }
    }
    ...
    async fn record_messages_delegates_to_uc() {
        ...
        let g = grpc(uc.clone());
        let _ = g.record_messages(Request::new(proto::RecordMessagesRequest {
            guild_id: "g".into(), user_id: "u".into(), username: "a".into(), count: 50,
        })).await.unwrap();
    ...
    async fn record_voice_delegates_to_uc() {
        let uc = Arc::new(MockStatsUc::default());
        let g = grpc(uc.clone());
        let _ = g.record_voice(Request::new(proto::RecordVoiceRequest {
            guild_id: "g".into(), user_id: "u".into(), username: "a".into(), seconds: 1800,
            channel_id: "c".into(), channel_name: "General".into(),
        })).await.unwrap();
    ...
    async fn get_user_stats_returns_some_when_uc_has_data() {
        ...
        *uc.user_stats_return.lock().unwrap() = Some(sample_user());
        let g = grpc(uc);
        let resp = g.get_user_stats(Request::new(proto::GetUserStatsRequest {
    ...
    async fn get_guild_overview_delegates() {
        let uc = Arc::new(MockStatsUc::default());
        let g = grpc(uc);
        let resp = g.get_guild_overview(Request::new(proto::GetGuildOverviewRequest {
            guild_id: "g1".into(),
        })).await.unwrap();
