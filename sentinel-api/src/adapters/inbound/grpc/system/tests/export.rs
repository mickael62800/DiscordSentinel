...
impl MockExportUc {
    fn ok(data: &str, count: usize) -> Self {
        Self {
            calls: Mutex::new(vec![]),
            result: Mutex::new(Ok(ExportResult { data: data.into(), row_count: count })),
        }
    }
...
impl ExecuteExportUseCase for MockExportUc {
    async fn execute(&self, g: &str, j: &str, f: &str, max: i64) -> Result<ExportResult, DomainError> {
        self.calls.lock().unwrap().push((g.into(), j.into(), f.into(), max));
        match &*self.result.lock().unwrap() {
            Ok(r) => Ok(ExportResult { data: r.data.clone(), row_count: r.row_count }),
            Err(DomainError::ValidationError(msg)) => Err(DomainError::ValidationError(msg.clone())),
...
async fn empty_guild_id_returns_invalid_argument() {
    let grpc = ExportGrpc { uc: Arc::new(MockExportUc::ok("", 0)) };
    let err = grpc.execute_export(make_req("", "infractions")).await.unwrap_err();
...
async fn successful_export_returns_data_and_count() {
    ...
    let resp = grpc.execute_export(make_req("g1", "infractions")).await.unwrap();
...
async fn uc_error_is_mapped_via_domain_to_status() {
    let grpc = ExportGrpc {
        uc: Arc::new(MockExportUc::err(DomainError::ValidationError("bad format".into()))),
    };
    let err = grpc.execute_export(make_req("g1", "infractions")).await.unwrap_err();
