use super::*;
    ...
    impl AnalyzeImageUseCase for MockAnalyzeImage {
        async fn analyze_image(&self, cmd: AnalyzeImageCommand) -> Result<ImageAnalysis, DomainError> {
            ...
            Ok(ImageAnalysis {
                ...
                duration: None,
                classifications: vec![
                    ImageClassification { label: "nudity".into(), confidence: 0.65 },
                ],
            })
        }
    }
    ...
    async fn analyze_image_delegates_to_uc() {
        ...
        let resp = g.analyze_image(Request::new(proto::AnalyzeImageRequest {
            guild_id: "g".into(),
            channel_id: "c".into(),
            user_id: "u".into(),
            username: "alice".into(),
    ...
    async fn analyze_image_empty_bytes_still_delegated() {
        ...
        let uc = Arc::new(MockAnalyzeImage::default());
        let g = ImagesGrpc { uc: uc.clone() };
        let _ = g.analyze_image(Request::new(proto::AnalyzeImageRequest {
            guild_id: "g".into(),
            channel_id: "c".into(),
            user_id: "u".into(),
            username: "a".into(),
