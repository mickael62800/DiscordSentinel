use super::*;
    ...
    use sentinel_core::domain::entities::community::voice_channel::VoiceChannelConfig;
    ...
    impl ManageVoiceChannelsUseCase for MockVoiceUc {
        async fn list_all_channels(&self) -> Result<Vec<VoiceChannel>, DomainError> { Ok(vec![]) }
        ...
        async fn list_invite_links(&self, _: &str) -> Result<Vec<VoiceChannelInviteLink>, DomainError> { Ok(vec![]) }
        ...
        async fn get_voice_config(&self, _g: &str) -> Result<VoiceChannelConfig, DomainError> {
            Ok(self.config.lock().unwrap().clone().unwrap_or(VoiceChannelConfig {
    ...
    fn grpc(uc: Arc<MockVoiceUc>) -> VoiceChannelsGrpc { VoiceChannelsGrpc { uc } }

    #[tokio::test]
    async fn list_channels_returns_all() {
        let uc = Arc::new(MockVoiceUc::default());
        uc.channels.lock().unwrap().push(sample_channel(VoiceChannelKind::Public));
        let g = grpc(uc);
        let resp = g.list_channels(Request::new(proto::ListChannelsRequest {
            guild_id: "g".into(),
        })).await.unwrap();
    ...
    async fn create_channel_delegates_command() {
        let uc = Arc::new(MockVoiceUc::default());
        let g = grpc(uc.clone());
        let _ = g.create_channel(Request::new(proto::CreateChannelRequest {
            guild_id: "g".into(),
            owner_id: "o".into(),
    ...
    async fn delete_channel_delegates_id() {
        let uc = Arc::new(MockVoiceUc::default());
        let g = grpc(uc.clone());
        let _ = g.delete_channel(Request::new(proto::DeleteChannelRequest {
            channel_id: "ch1".into(),
        })).await.unwrap();
    ...
    async fn update_channel_unwraps_optional_wrappers() {
        let uc = Arc::new(MockVoiceUc::default());
        let g = grpc(uc.clone());
        let _ = g.update_channel(Request::new(proto::UpdateChannelRequest {
            channel_id: "c".into(),
    ...
    async fn get_channel_not_found_returns_none() {
        ...
        let resp = g.get_channel(Request::new(proto::GetChannelRequest {
    ...
    async fn get_channel_found_returns_detail_with_co_admins() {
        ...
        *uc.detail.lock().unwrap() = Some(VoiceChannelDetail {
        ...
        let _ = uc.detail.lock().unwrap().as_ref().unwrap().channel.channel_id.clone();
    ...
    async fn transfer_ownership_delegates() {
        let uc = Arc::new(MockVoiceUc::default());
        let g = grpc(uc.clone());
        let _ = g.transfer_ownership(Request::new(proto::TransferOwnershipRequest {
            channel_id: "ch".into(),
    ...
    async fn add_co_admin_delegates() {
        let uc = Arc::new(MockVoiceUc::default());
        let g = grpc(uc.clone());
        let _ = g.add_co_admin(Request::new(proto::AddCoAdminRequest {
            channel_id: "ch".into(),
            user_id: "co".into(),
            user_name: "CoAdmin".into(),
        })).await.unwrap();
        assert_eq!(uc.add_co_admin_calls.lock().unwrap()[0].user_name, "CoAdmin");
    }
    ...
    async fn add_to_whitelist_delegates() {
        let uc = Arc::new(MockVoiceUc::default());
        let g = grpc(uc.clone());
        let _ = g.add_to_whitelist(Request::new(proto::AddToWhitelistRequest {
            guild_id: "g".into(),
            owner_id: "o".into(),
            target_id: "t".into(),
    ...
    async fn ban_from_channel_delegates_with_duration() {
        let uc = Arc::new(MockVoiceUc::default());
        let g = grpc(uc.clone());
        let _ = g.ban_from_channel(Request::new(proto::BanFromChannelRequest {
            channel_id: "ch".into(),
            user_id: "u".into(),
    ...
    async fn get_voice_config_returns_proto() {
        let g = grpc(Arc::new(MockVoiceUc::default()));
        let resp = g.get_voice_config(Request::new(proto::GetVoiceConfigRequest {
            guild_id: "g".into(),
        })).await.unwrap();
    ...
    async fn list_themes_returns_all() {
        ...
        let g = grpc(uc);
        let resp = g.list_themes(Request::new(proto::ListThemesRequest {
            guild_id: "g".into(),
        })).await.unwrap();
