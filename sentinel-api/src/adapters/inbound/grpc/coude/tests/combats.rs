...
impl ManageCoudeCombatsUseCase for MockCombatsUc {
    async fn list(&self, g: &str, s: Option<&str>, l: i64) -> Result<Vec<Combat>, DomainError> {
        self.list_calls
            .lock()
            .unwrap()
            .push((g.into(), s.map(String::from), l));
...
impl ResolveCombatNowUseCase for MockResolveNow {
    async fn resolve_now(&self, id: Uuid) -> Result<ResolveCombatNowOutput, DomainError> {
        ...
        Ok(self
            ...
            .unwrap()
            .clone()
            .unwrap_or(ResolveCombatNowOutput {
...
async fn list_default_limit_when_zero() {
    ...
    let g = grpc(uc.clone());
    let _ = g
        .list(Request::new(proto::ListCombatsRequest {
            guild_id: "g".into(),
            status: None,
            limit: 0,
        }))
...
async fn get_rejects_invalid_uuid() {
    ...
    let err = g
        .get(Request::new(proto::GetCombatRequest {
            id: "not-a-uuid".into(),
        }))
        .await
        .unwrap_err();
...
async fn get_pending_for_attacker_some_when_present() {
    ...
    *uc.pending_attacker.lock().unwrap() = Some(sample_combat());
    let g = grpc(uc);
    let resp = g
        .get_pending_for_attacker(Request::new(proto::GetPendingForAttackerRequest {
            guild_id: "g".into(),
            attacker_id: "a".into(),
        }))
        .await
...
async fn get_pending_for_defender_some_when_present() {
    ...
    *uc.pending_defender.lock().unwrap() = Some(sample_combat());
    let g = grpc(uc);
    let resp = g
        .get_pending_for_defender(Request::new(proto::GetPendingForDefenderRequest {
            guild_id: "g".into(),
            defender_id: "d".into(),
        }))
        .await
...
async fn list_expired_pending_returns_all() {
    ...
    uc.expired_pending
        .lock()
        .unwrap()
        .extend(vec![sample_combat(), sample_combat()]);
    let g = grpc(uc);
    let resp = g
        .list_expired_pending(Request::new(proto::Empty {}))
        .await
        .unwrap();
...
async fn get_betting_for_participant_returns_maybe_combat() {
    ...
    *uc.betting_participant.lock().unwrap() = Some(sample_combat());
    let g = grpc(uc);
    let resp = g
        .get_betting_for_participant(Request::new(proto::GetBettingForParticipantRequest {
            guild_id: "g".into(),
            user_id: "u".into(),
        }))
        .await
...
async fn create_delegates_all_fields() {
    ...
    let g = grpc(uc.clone());
    let _ = g
        .create(Request::new(proto::CreateCombatRequest {
            guild_id: "g".into(),
            channel_id: Some("c".into()),
            attacker_id: "a".into(),
            attacker_name: "Atk".into(),
...
async fn cancel_rejects_invalid_uuid() {
    let g = grpc(Arc::new(MockCombatsUc::default()));
    let err = g
        .cancel(Request::new(proto::CancelCombatRequest {
            id: "bad".into(),
        }))
        .await
        .unwrap_err();
...
async fn resolve_rejects_invalid_uuid() {
    let g = grpc(Arc::new(MockCombatsUc::default()));
    let err = g
        .resolve(Request::new(proto::ResolveCombatRequest {
            id: "bad".into(),
...
async fn set_betting_rejects_invalid_uuid() {
    let g = grpc(Arc::new(MockCombatsUc::default()));
    let err = g
        .set_betting(Request::new(proto::SetBettingRequest {
            id: "bad".into(),
            message_id: "m".into(),
        }))
        .await
...
async fn expire_rejects_invalid_uuid() {
    let g = grpc(Arc::new(MockCombatsUc::default()));
    let err = g
        .expire(Request::new(proto::ExpireCombatRequest {
            id: "bad".into(),
        }))
        .await
        .unwrap_err();
...
async fn set_defender_special_rejects_invalid_uuid() {
    let g = grpc(Arc::new(MockCombatsUc::default()));
    let err = g
        .set_defender_special(Request::new(proto::SetDefenderSpecialRequest {
            id: "bad".into(),
...
async fn set_defender_special_valid_delegates() {
    ...
    let _ = g
        ...
        .unwrap();
    assert_eq!(
        uc.set_defender_calls.lock().unwrap()[0],
        (id, "explosion".into())
    );
}
...
async fn resolve_betting_batch_empty_returns_empty() {
    ...
    let resp = g
        .resolve_betting_batch(Request::new(proto::Empty {}))
        .await
        .unwrap();
...
async fn resolve_betting_batch_maps_outputs() {
    ...
    rb.outputs
        .lock()
        .unwrap()
        .push(ResolvedBettingCombatOutput {
            combat_id: "c1".into(),
            guild_id: "g".into(),
...
async fn expire_combats_batch_maps_outputs() {
    ...
    let g = grpc_full(uc, rb, eb, rn);
    let resp = g
        .expire_combats_batch(Request::new(proto::Empty {}))
        .await
        .unwrap();
    let inner = resp.into_inner();
    assert_eq!(inner.combats.len(), 1);
...
async fn resolve_combat_now_rejects_invalid_uuid() {
    ...
    let g = grpc_full(uc, rb, eb, rn);
    let err = g
        .resolve_combat_now(Request::new(proto::ResolveCombatNowRequest {
            combat_id: "bad".into(),
        }))
        .await
        .unwrap_err();
