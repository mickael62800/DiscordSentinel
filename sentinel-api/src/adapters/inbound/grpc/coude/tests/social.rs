...
impl ManageCoudeCashboxUseCase for MockCashboxUc {
    ...
    async fn list_redistributions(
        ...
        Vec<sentinel_core::domain::entities::coude::cashbox::CashboxRedistribution>,
        DomainError,
    > {
        unimplemented!()
    }
    async fn list_entries(
        ...
        Vec<sentinel_core::domain::entities::coude::cashbox::CashboxRedistributionEntry>,
        DomainError,
    > {
        unimplemented!()
    }
...
impl ManageCoudeTauntsUseCase for MockTauntsUc {
    ...
    async fn set_channel(&self, g: &str, c: Option<&str>) -> Result<(), DomainError> {
        self.set_channel_calls
            .lock()
            .unwrap()
            .push((g.into(), c.map(|s| s.to_string())));
        Ok(())
    }
...
impl ManageCoudeHeistUseCase for MockHeistUc {
    ...
    async fn attempt_heist(&self, _: &str, _: &str) -> Result<HeistOutcome, DomainError> {
        Ok(self
            ...
            .unwrap()
            .clone()
            .unwrap_or(HeistOutcome {
                success: false,
                chance_percent: 0,
                cashbox_total_before: 0,
                amount_stolen: 0,
                tools_consumed: vec![],
...
fn defaults() -> (
    ...
    (
        Arc::new(MockSocialUc::default()),
        Arc::new(MockCatalogUc::default()),
        Arc::new(MockCashboxUc::default()),
        Arc::new(MockTauntsUc::default()),
...
fn category_mapping() {
    ...
    assert!(matches!(
        proto_to_leaderboard_category(P::Thieves as i32),
        LeaderboardCategory::Thieves
    ));
    assert!(matches!(
        proto_to_leaderboard_category(P::Cowards as i32),
        LeaderboardCategory::Cowards
    ));
    assert!(matches!(
        proto_to_leaderboard_category(P::Chaos as i32),
        LeaderboardCategory::Chaos
    ));
    assert!(matches!(
        proto_to_leaderboard_category(P::Level as i32),
        LeaderboardCategory::Level
    ));
    assert!(matches!(
        proto_to_leaderboard_category(P::Unspecified as i32),
        LeaderboardCategory::Richest
    ));
    assert!(matches!(
        proto_to_leaderboard_category(99999),
        LeaderboardCategory::Richest
    ));
}
...
async fn check_cooldown_none_returns_none() {
    ...
    let g = mk(s, c, cb, t, h);
    let resp = g
        .check_cooldown(Request::new(proto::CheckCooldownRequest {
            guild_id: "g".into(),
            user_id: "u".into(),
            action: "daily".into(),
        }))
...
async fn leaderboard_maps_entries() {
    ...
    let g = mk(s, c, cb, t, h);
    let resp = g
        .leaderboard(Request::new(proto::LeaderboardRequest {
            guild_id: "g".into(),
            category: 0,
            limit: 10,
        }))
...
async fn list_active_events_maps() {
    ...
    let g = mk(s, c, cb, t, h);
    let resp = g
        .list_active_events(Request::new(proto::ListActiveEventsRequest {
            guild_id: "g".into(),
        }))
        .await
        .unwrap();
...
async fn current_season_maps() {
    ...
    *s.season_return.lock().unwrap() = Some(Season {
    ...
    let g = mk(s, c, cb, t, h);
    let resp = g
        .current_season(Request::new(proto::CurrentSeasonRequest {
            guild_id: "g".into(),
        }))
        .await
        .unwrap();
...
async fn get_catalog_maps_all_collections() {
    ...
    *c.catalog.lock().unwrap() = Some(Catalog {
        classes: vec![ClassInfo {
            ...
            description: "d".into(),
            passif_key: "pk".into(),
            passif_description: "pd".into(),
            passif_reveal: "pr".into(),
        }],
        ...
        anti_theft_items: vec![AntiTheftItemInfo {
            key: "lock".into(),
            block_chance_percent: 20,
        }],
        max_level: 50,
        hp_base: 100,
        hp_per_def: 10,
    ...
    let g = mk(s, c, cb, t, h);
    let resp = g
        .get_catalog(Request::new(proto::Empty {}))
        .await
        .unwrap()
        .into_inner();
...
async fn get_cashbox_maps() {
    ...
    *cb.cashbox.lock().unwrap() = Some(Cashbox {
    ...
    let g = mk(s, c, cb, t, h);
    let resp = g
        .get_cashbox(Request::new(proto::GetCashboxRequest {
            guild_id: "g".into(),
        }))
        .await
        .unwrap()
...
async fn redistribute_cashbox_none_returns_not_executed() {
    ...
    let g = mk(s, c, cb, t, h);
    let resp = g
        .redistribute_cashbox(Request::new(proto::GetCashboxRequest {
            guild_id: "g".into(),
        }))
        .await
        .unwrap()
...
async fn redistribute_cashbox_some_returns_winners() {
    ...
    *cb.redistribute_return.lock().unwrap() = Some(RedistributionOutcome {
        ...
        total_amount: 1000,
        winners: vec![
            ("u1".into(), "Alice".into(), 600),
            ("u2".into(), "Bob".into(), 400),
        ],
    });
    let g = mk(s, c, cb, t, h);
    let resp = g
...
async fn redistribute_due_maps_and_clamps_negative() {
    ...
    let g = mk(s, c, cb, t, h);
    let resp = g
        .redistribute_due_cashboxes(Request::new(proto::RedistributeDueRequest {
...
async fn deposit_cashbox_rejects_unspecified_source() {
    ...
    let g = mk(s, c, cb, t, h);
    let err = g
        .deposit_cashbox(Request::new(proto::DepositCashboxRequest {
            guild_id: "g".into(),
            amount: 100,
...
fn proto_source_mapping_covers_all_variants() {
    ...
    assert!(matches!(
        proto_source_to_domain(P::CashboxSourceShopPurchase as i32),
        Some(CashboxSource::ShopPurchase)
    ));
    assert!(matches!(
        proto_source_to_domain(P::CashboxSourceInsurancePurchase as i32),
        Some(CashboxSource::InsurancePurchase)
    ));
    assert!(matches!(
        proto_source_to_domain(P::CashboxSourceProtectionPurchase as i32),
        Some(CashboxSource::ProtectionPurchase)
    ));
    assert!(matches!(
        proto_source_to_domain(P::CashboxSourceBoostPurchase as i32),
        Some(CashboxSource::BoostPurchase)
    ));
    assert!(matches!(
        proto_source_to_domain(P::CashboxSourceClassChangeCost as i32),
        Some(CashboxSource::ClassChangeCost)
    ));
    assert!(matches!(
        proto_source_to_domain(P::CashboxSourceResetStatsCost as i32),
        Some(CashboxSource::ResetStatsCost)
    ));
    assert!(matches!(
        proto_source_to_domain(P::CashboxSourceDonationTax as i32),
        Some(CashboxSource::DonationTax)
    ));
    assert!(matches!(
        proto_source_to_domain(P::CashboxSourceCowardicePenalty as i32),
        Some(CashboxSource::CowardicePenalty)
    ));
    assert!(matches!(
        proto_source_to_domain(P::CashboxSourceBetCommission as i32),
...
async fn track_steal_victim_none() {
    ...
    let g = mk(s, c, cb, t, h);
    let resp = g
        .track_steal_victim(Request::new(proto::TrackStealVictimRequest {
            guild_id: "g".into(),
            victim_id: "v".into(),
        }))
        .await
...
async fn track_steal_defended_delegates() {
    ...
    .unwrap();
    assert_eq!(
        t.defended_calls.lock().unwrap()[0],
        ("g".into(), "u".into())
    );
}
...
async fn get_taunts_config_maps() {
    ...
    *t.config.lock().unwrap() = Some(TauntsConfig {
    ...
    let g = mk(s, c, cb, t, h);
    let resp = g
        .get_taunts_config(Request::new(proto::GetTauntsConfigRequest {
            guild_id: "g".into(),
        }))
        .await
        .unwrap()
...
async fn set_taunts_opt_out_delegates() {
    ...
    .unwrap();
    assert_eq!(
        t.set_opt_out_calls.lock().unwrap()[0],
        ("g".into(), "u".into(), true)
    );
}
...
async fn attempt_heist_maps_success() {
    ...
    *h.outcome.lock().unwrap() = Some(HeistOutcome {
        success: true,
        chance_percent: 15,
        cashbox_total_before: 1000,
        amount_stolen: 300,
        tools_consumed: vec!["lockpick".into()],
        prison_released_at: None,
    });
    let g = mk(s, c, cb, t, h);
    let resp = g
        .attempt_heist(Request::new(proto::UserInGuildRequest {
            guild_id: "g".into(),
            user_id: "u".into(),
        }))
        .await
...
async fn attempt_heist_maps_failure_with_prison() {
    ...
    *h.outcome.lock().unwrap() = Some(HeistOutcome {
        ...
        amount_stolen: 0,
        tools_consumed: vec![],
        prison_released_at: Some(release),
    });
    let g = mk(s, c, cb, t, h);
    let resp = g
        .attempt_heist(Request::new(proto::UserInGuildRequest {
...
async fn get_heist_cooldown_maps() {
    ...
    *h.cooldown.lock().unwrap() = Some(HeistCooldownStatus {
    ...
    let g = mk(s, c, cb, t, h);
    let resp = g
        .get_heist_cooldown(Request::new(proto::UserInGuildRequest {
            guild_id: "g".into(),
            user_id: "u".into(),
        }))
        .await
...
async fn get_prison_status_maps() {
    ...
    *h.prison.lock().unwrap() = Some(PrisonStatusInfo {
        in_prison: true,
        released_at: Some(rel),
        reason: Some("heist_fail".into()),
    });
    let g = mk(s, c, cb, t, h);
    let resp = g
        .get_prison_status(Request::new(proto::UserInGuildRequest {
            guild_id: "g".into(),
            user_id: "u".into(),
        }))
        .await
...
async fn trigger_daily_chaos_none_returns_not_triggered() {
    ...
    let g = mk(s, c, cb, t, h);
    let resp = g
        .trigger_daily_chaos(Request::new(proto::TriggerDailyChaosRequest {
            guild_id: "g".into(),
        }))
        .await
        .unwrap()
