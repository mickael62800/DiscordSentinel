...
async fn get_or_create_player_delegates() {
    let g = grpc(
        Arc::new(MockPlayersUc::default()),
        Arc::new(MockWalletUc::default()),
    );
    let resp = g
        .get_or_create_player(Request::new(proto::GetOrCreatePlayerRequest {
            guild_id: "g1".into(),
            user_id: "u1".into(),
            username: "Alice".into(),
        }))
...
async fn get_player_success() {
    ...
    *players.player.lock().unwrap() = Some(make_player());
    ...
    let resp = g
        .get_player(Request::new(proto::GetPlayerRequest {
            guild_id: "g".into(),
            user_id: "u".into(),
        }))
        .await
...
async fn update_player_class_rejects_invalid_class() {
    ...
    let err = g
        .update_player_class(Request::new(proto::UpdatePlayerClassRequest {
            guild_id: "g".into(),
            user_id: "u".into(),
            class: "wizard".into(),
        }))
...
async fn adjust_coins_positive_credits_wallet() {
    ...
    let _ = g
        .adjust_coins(Request::new(proto::AdjustCoinsRequest {
            guild_id: "g".into(),
            user_id: "u".into(),
            delta: 500,
        }))
...
async fn update_hp_delegates_to_uc() {
    ...
    let g = grpc(players.clone(), Arc::new(MockWalletUc::default()));
    let _ = g
        .update_hp(Request::new(proto::UpdateHpRequest {
            guild_id: "g".into(),
            user_id: "u".into(),
            hp_current: 50,
            hp_max: 100,
...
async fn hp_regen_tick_returns_updated_count() {
    ...
    *players.regen_return.lock().unwrap() = 42;
    let g = grpc(players, Arc::new(MockWalletUc::default()));
    let resp = g
        .hp_regen_tick(Request::new(proto::HpRegenTickRequest {
            rate_0_25: 1.0,
            rate_25_50: 2.0,
            rate_50_75: 3.0,
            rate_75_100: 4.0,
