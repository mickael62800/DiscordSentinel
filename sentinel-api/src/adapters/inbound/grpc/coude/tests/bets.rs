...
async fn place_delegates_to_uc() {
    ...
    let resp = g
        .place(Request::new(proto::PlaceBetRequest {
            guild_id: "g1".into(),
...
async fn list_for_combat_returns_bets_from_uc() {
    ...
    let g = grpc(uc);
    let resp = g
        .list_for_combat(Request::new(proto::ListForCombatRequest {
...
async fn resolve_with_winner_delegates_to_uc() {
    ...
    let combat_id = Uuid::new_v4();
    let resp = g
        .resolve(Request::new(proto::ResolveBetsRequest {
            combat_id: combat_id.to_string(),
...
async fn refund_returns_summary() {
    ...
    let combat_id = Uuid::new_v4();
    let resp = g
        .refund(Request::new(proto::RefundBetsRequest {
            combat_id: combat_id.to_string(),
        }))
        .await
        .unwrap();
