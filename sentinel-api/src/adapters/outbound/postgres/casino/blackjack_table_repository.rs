...
impl BlackjackTableRepository for PgBlackjackTableRepository {
    ...
    async fn get_status_and_guild(&self, table_id: &str) -> Result<Option<(String, String)>, sentinel_core::domain::errors::DomainError> {
    ...
    async fn count_players(&self, table_id: &str) -> Result<i64, sentinel_core::domain::errors::DomainError> {
    ...
    async fn add_player(&self, table_id: &str, user_id: &str, user_name: &str) -> Result<(), sentinel_core::domain::errors::DomainError> {
        sqlx::query("INSERT INTO blackjack_table_players (table_id, user_id, user_name) VALUES ($1::uuid, $2, $3) ON CONFLICT DO NOTHING")
    ...
    async fn touch_activity(&self, table_id: &str) -> Result<(), sentinel_core::domain::errors::DomainError> {
    ...
    async fn close(&self, table_id: &str) -> Result<(), sentinel_core::domain::errors::DomainError> {
        sqlx::query("UPDATE blackjack_tables SET status = 'closed' WHERE id = $1::uuid AND status = 'open'")
            .bind(table_id).execute(&self.pool).await.map_err(pg_err)?;
        Ok(())
    }

    async fn list_games(&self, table_id: &str) -> Result<Vec<serde_json::Value>, sentinel_core::domain::errors::DomainError> {
