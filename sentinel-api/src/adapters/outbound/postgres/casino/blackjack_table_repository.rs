...
pub struct PgBlackjackTableRepository {
    pool: PgPool,
}
...
impl BlackjackTableRepository for PgBlackjackTableRepository {
    ...
    ) -> Result<BlackjackTable, sentinel_core::domain::errors::DomainError> {
    ...
    async fn get_status_and_guild(
        &self,
        table_id: &str,
    ) -> Result<Option<(String, String)>, sentinel_core::domain::errors::DomainError> {
        let row: Option<(String, String)> =
            sqlx::query_as("SELECT status, guild_id FROM blackjack_tables WHERE id = $1::uuid")
                .bind(table_id)
    ...
    async fn count_players(
        &self,
        table_id: &str,
    ) -> Result<i64, sentinel_core::domain::errors::DomainError> {
    ...
    async fn list_players(
        &self,
        table_id: &str,
    ) -> Result<Vec<BlackjackTablePlayer>, sentinel_core::domain::errors::DomainError> {
        ...
        Ok(rows
            .into_iter()
            .map(|(user_id, user_name, joined_at)| BlackjackTablePlayer {
                user_id,
                user_name,
                joined_at,
            })
    ...
    async fn find_open_by_channel(
        &self,
        channel_id: &str,
    ) -> Result<Option<BlackjackTable>, sentinel_core::domain::errors::DomainError> {
    ...
    async fn get_guild_id(
        ...
        let row: Option<(String,)> =
            sqlx::query_as("SELECT guild_id FROM blackjack_tables WHERE id = $1::uuid")
                .bind(table_id)
                .fetch_optional(&self.pool)
                .await
                .map_err(pg_err)?;
    ...
    async fn list_games(
        &self,
        table_id: &str,
    ) -> Result<Vec<serde_json::Value>, sentinel_core::domain::errors::DomainError> {
