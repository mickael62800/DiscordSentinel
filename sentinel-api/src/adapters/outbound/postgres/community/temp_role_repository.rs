...
impl TempRoleRepository for PgTempRoleRepository {
    async fn create(&self, guild_id: &str, user_id: &str, role_id: &str, expires_at: &str) -> Result<(), sentinel_core::domain::errors::DomainError> {
        sqlx::query(
        ...
        .bind(guild_id).bind(user_id).bind(role_id).bind(expires_at)
        .execute(&self.pool).await.map_err(pg_err)?;
        Ok(())
    }

    async fn list_active(&self, guild_id: &str) -> Result<Vec<TempRole>, sentinel_core::domain::errors::DomainError> {
        #[derive(sqlx::FromRow)]
        struct Row { id: uuid::Uuid, guild_id: GuildId, user_id: UserId, role_id: RoleId, expires_at: chrono::DateTime<chrono::Utc>, created_at: chrono::DateTime<chrono::Utc> }
        ...
        Ok(rows.into_iter().map(|r| TempRole {
            id: r.id, guild_id: r.guild_id.into(), user_id: r.user_id.into(),
            role_id: r.role_id.into(), expires_at: r.expires_at, created_at: r.created_at,
        }).collect())
    }

    async fn delete(&self, guild_id: &str, user_id: &str, role_id: &str) -> Result<(), sentinel_core::domain::errors::DomainError> {
        sqlx::query("DELETE FROM temp_roles WHERE guild_id = $1 AND user_id = $2 AND role_id = $3")
            .bind(guild_id).bind(user_id).bind(role_id)
            .execute(&self.pool).await.map_err(pg_err)?;
        Ok(())
    }
}
