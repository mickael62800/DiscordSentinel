//! Consommateur d'evenements Redis Streams du bot Nexus.
//!
//! Porte depuis `sentinel-bot/src/shared/event_bus.rs`, en version consumer
//! seul : nexus-api publie sur la stream `nexus:events`, le bot consomme via
//! un consumer group durable.
//!
//! Garanties reprises telles quelles :
//!   - at-least-once (XREADGROUP + XACK, entries persistees jusqu'a l'ACK) ;
//!   - idempotence (dedup bornee des IDs deja traites, cf. `ProcessedIds`) ;
//!   - rattrapage des pending d'un consumer crashe (XAUTOCLAIM periodique) ;
//!   - reconnexion automatique.
//!
//! Format d'une entry : un champ `payload` contenant le JSON
//! `{"event": ..., "data": ...}`.
    ...
    Fut: Future<Output = ()> + Send,
{
    ...
    loop {
        // XREADGROUP STREAMS nexus:events >  (> = seulement les nouveaux)
