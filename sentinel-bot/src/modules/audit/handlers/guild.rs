...
pub async fn handle_update(
    ...
    if let Some(ref old_guild) = old {
        if old_guild.name != new_incomplete.name {
            changes.push(format!(
                "nom: {} -> {}",
                old_guild.name, new_incomplete.name
            ));
        }
        ...
        if old_guild.verification_level != new_incomplete.verification_level {
            changes.push(format!(
                "niveau verification: {:?} -> {:?}",
                old_guild.verification_level, new_incomplete.verification_level
            ));
        }
    ...
    log(
        ...
        "warn",
        &gid,
        &format!("Serveur modifie : {}", changes.join(", ")),
    )
    .await;

    let mut evt = audit_event::simple(gid, "guild_update").with_details(serde_json::json!({
        "changes": changes,
    }));
