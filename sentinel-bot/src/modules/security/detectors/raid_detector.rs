...
/// Detecteur de raid base sur le nombre de joins dans une fenetre de temps.
/// Thread-safe grace a DashMap.
...
impl RaidDetector {
    ...
    /// Retourne le nombre de joins recents pour un guild.
    ...
    /// Reinitialise les compteurs d'un guild (apres lockdown par ex).
...
mod tests {
    ...
    fn test_raid_at_threshold() {
        ...
        assert!(!detector.record_join(guild));
        assert!(!detector.record_join(guild));
        assert!(detector.record_join(guild)); // 3eme = raid
    }
    ...
    fn test_different_guilds_independent() {
        ...
        assert!(detector.record_join(guild_a)); // 2eme pour A
        assert!(detector.record_join(guild_b)); // 2eme pour B
    }
