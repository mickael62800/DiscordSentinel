...
fn self_mute_in_same_channel_skips_handlers() {
    ...
    let result =
        should_run_leave_handlers(Some(ch(TEMP_A)), Some(ch(TEMP_A)), ch(PUBLIC), ch(PRIVATE));
...
fn user_leaves_voice_completely_runs_handlers() {
    ...
    let result = should_run_leave_handlers(Some(ch(TEMP_A)), None, ch(PUBLIC), ch(PRIVATE));
...
fn user_moves_to_another_channel_runs_handlers_on_old() {
    ...
    let result =
        should_run_leave_handlers(Some(ch(TEMP_A)), Some(ch(TEMP_B)), ch(PUBLIC), ch(PRIVATE));
    assert_eq!(result, Some(ch(TEMP_A)));
}
...
fn user_joins_first_time_skips_handlers() {
    ...
    let result = should_run_leave_handlers(None, Some(ch(TEMP_A)), ch(PUBLIC), ch(PRIVATE));
    assert_eq!(result, None);
}
...
fn leaving_public_creator_skips_handlers() {
    ...
    let result =
        should_run_leave_handlers(Some(ch(PUBLIC)), Some(ch(TEMP_A)), ch(PUBLIC), ch(PRIVATE));
    assert_eq!(result, None);
}
...
fn leaving_private_creator_skips_handlers() {
    ...
    let result =
        should_run_leave_handlers(Some(ch(PRIVATE)), Some(ch(TEMP_A)), ch(PUBLIC), ch(PRIVATE));
    assert_eq!(result, None);
}
...
fn moving_from_temp_to_creator_lobby_still_runs_handlers() {
    ...
    let result =
        should_run_leave_handlers(Some(ch(TEMP_A)), Some(ch(PUBLIC)), ch(PUBLIC), ch(PRIVATE));
    assert_eq!(result, Some(ch(TEMP_A)));
}
