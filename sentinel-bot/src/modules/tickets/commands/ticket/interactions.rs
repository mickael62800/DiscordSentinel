...
pub async fn handle_vocal_button(ctx: &Context, component: &ComponentInteraction) {
        ...
        if let Err(e) = component
            .create_response(
                ...
                CreateInteractionResponse::Message(
                    CreateInteractionResponseMessage::new()
                        .content(
                            "Seuls les administrateurs et moderateurs peuvent proposer un vocal.",
                        )
                        .ephemeral(true),
                ),
            )
    ...
    if let Ok(channels) = guild_id.channels(&ctx.http).await {
        if channels
            .values()
            .any(|c| c.kind == ChannelType::Voice && c.name == vocal_name)
        {
            if let Err(e) = component
                .create_response(
                    &ctx.http,
    ...
    if let Ok(messages) = component
        .channel_id
        .messages(&ctx.http, serenity::all::GetMessages::new().limit(10))
        .await
    {
...
pub async fn handle_vocal_user_accept(ctx: &Context, component: &ComponentInteraction) {
    ...
    let category_id = guild_channel.as_ref().and_then(|c| c.parent_id);
