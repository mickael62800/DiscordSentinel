...
async function fetchWallets() {
  ...
  error.value = null;
  try {
    wallets.value = await invoke<Wallet[]>("wallet_list", {
...
async function credit(wallet: Wallet) {
  ...
  try {
    await invoke<Wallet>("wallet_credit", {
      guildId: selectedGuildId.value,
      userId: wallet.user_id,
      amount,
      description: "Credit admin desktop",
    });
...
async function debit(wallet: Wallet) {
  ...
  busy.value = wallet.user_id;
  try {
    await invoke<Wallet>("wallet_debit", {
      guildId: selectedGuildId.value,
      userId: wallet.user_id,
      amount,
      description: "Debit admin desktop",
    });
...
async function resetOne(wallet: Wallet) {
  ...
  busy.value = wallet.user_id;
  try {
    await invoke<Wallet>("wallet_reset", {
      guildId: selectedGuildId.value,
      userId: wallet.user_id,
      newBalance,
    });
...
async function resetAll() {
  ...
  loading.value = true;
  try {
    const affected = await invoke<number>("wallet_reset_all", {
      guildId: selectedGuildId.value,
      newBalance: resetAllBalance.value,
    });
