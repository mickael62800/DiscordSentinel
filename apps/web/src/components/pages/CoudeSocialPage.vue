...
<template>
  <div class="page">
    ...
    <div v-if="lookupUserId && !loading" class="grid">
      <!-- Curses -->
      <section class="card">
        ...
        <div v-else class="curse-card">
          ...
          <p>
            <em>Lancée par</em> <code>{{ curse.source_id }}</code> le
            {{ formatDate(curse.created_at) }}.
          </p>
          <p>Expire : {{ formatDate(curse.expires_at) }}</p>
          <button class="btn-warn" @click="liftCurse">
            🛡️ Lever (admin override)
          </button>
        </div>
      </section>
      ...
      <section class="card">
        ...
        <div v-else>
          <div class="kv">
            <span>Pot total</span>
            <strong>{{ bounty.total_amount.toLocaleString() }} coins</strong>
          </div>
          <div class="kv">
            <span>Statut</span>
            <span
              class="badge"
              :style="{ backgroundColor: statusColor(bounty.status) }"
            >{{ bounty.status }}</span>
          </div>
          <div class="kv">
            <span>Ouverte le</span>
            <span>{{ formatDate(bounty.opened_at) }}</span>
          </div>
          <div v-if="bounty.claimed_by" class="kv">
            <span>Claimée par</span>
            <code>{{ bounty.claimed_by }}</code>
          </div>
          <div v-if="bounty.claimed_at" class="kv">
            <span>Le</span>
            <span>{{ formatDate(bounty.claimed_at) }}</span>
          </div>
        </div>
      </section>
      ...
      <section class="card">
        ...
        <div v-else>
          <div class="kv">
            ...
            <span
              class="badge"
              :style="{ backgroundColor: statusColor(coalition.status) }"
            >{{ coalition.status }}</span>
          </div>
          <div class="kv">
            <span>Ouverte</span>
            <span>{{ formatDate(coalition.opened_at) }}</span>
          </div>
          <div class="kv">
            <span>Expire</span>
            <span>{{ formatDate(coalition.expires_at) }}</span>
          </div>
          <div v-if="coalition.broken_by" class="kv">
            <span>Cassée par</span>
            <code>{{ coalition.broken_by }}</code>
          </div>
      ...
      <section class="card">
        ...
        <ul v-else class="vendetta-list">
          <li v-for="v in vendettasAsChallenger" :key="v.id">
            ...
            <div class="kv">
              <span>Déclarée</span>
              <span>{{ formatDate(v.declared_at) }}</span>
            </div>
            <div class="kv">
              <span>Expire</span>
              <span>{{ formatDate(v.expires_at) }}</span>
            </div>
            <div v-if="v.resolved_at" class="kv">
              <span>Résolue</span>
              <span>{{ formatDate(v.resolved_at) }}</span>
            </div>
          </li>
        </ul>
      </section>
