...
impl InventoryRepository for MockRepo {
    ...
    async fn expire_insurance(&self, _: Uuid) -> Result<bool, DomainError> {
        Ok(true)
    }
}
...
async fn create_prime_rejects_negative_amount() {
    let svc = make_svc();
    let err = svc
        .create_prime(new_prime(-100, "t", "p"))
        .await
        .unwrap_err();
    assert!(matches!(err, DomainError::ValidationError(_)));
}
...
async fn create_prime_rejects_self_prime() {
    let svc = make_svc();
    let err = svc
        .create_prime(new_prime(100, "same", "same"))
        .await
        .unwrap_err();
    match err {
...
async fn create_prime_accepts_valid_request() {
    let svc = make_svc();
    let prime = svc
        .create_prime(new_prime(500, "victim", "hunter"))
        .await
        .unwrap();
...
async fn use_item_delegates_and_returns_value() {
    ...
    *repo.use_item_return.lock().unwrap() = true;
    ...
    assert_eq!(
        repo.use_calls.lock().unwrap()[0],
        ("g".into(), "u".into(), "masque".into())
    );
}
...
async fn claim_primes_delegates_args_and_returns_total() {
    ...
    *repo.claim_amount.lock().unwrap() = 2500;
    let svc = ManageCoudeInventoryService::new(repo.clone());
    let amount = svc
        .claim_primes("g", "t", "claimer", "Claimer Name")
        .await
        .unwrap();
    assert_eq!(amount, 2500);
    assert_eq!(
        repo.claim_calls.lock().unwrap()[0],
        (
            "g".into(),
            "t".into(),
            "claimer".into(),
...
async fn add_item_delegates_args_when_valid() {
    ...
    assert_eq!(
        repo.items_added.lock().unwrap()[0],
