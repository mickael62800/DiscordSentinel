...
impl MockPlayerRepo {
    fn insert(&self, p: Player) {
        self.players
            .lock()
            .unwrap()
            .insert(format!("{}:{}", p.guild_id, p.user_id), p);
    }
}
...
async fn resolve_batch_merges_due_and_stuck() {
    ...
    combat_repo
        ...
        .lock()
        .unwrap()
        .push(make_combat("a1", "b1", 50));
    combat_repo
        .stuck
        .lock()
        .unwrap()
        .push(make_combat("a2", "b2", 50));
...
async fn resolve_batch_skips_combat_when_player_missing() {
    ...
    combat_repo
        ...
        .lock()
        .unwrap()
        .push(make_combat("ghost", "def", 100));
...
async fn resolve_batch_giant_killer_underdog_scenario() {
    ...
    combat_repo
        ...
        .lock()
        .unwrap()
        .push(make_combat("weakling", "boss", 100));
