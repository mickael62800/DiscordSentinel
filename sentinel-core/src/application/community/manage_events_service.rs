...
use crate::domain::entities::community::event::{
    CommunityEvent, EventAnswer, UpsertEventCommand,
};
use crate::domain::errors::DomainError;
use crate::ports::inbound::community::manage_events::{
    EventWithParticipants, ManageEventsUseCase,
};
use crate::ports::outbound::community::event_repository::{EventRepository, EventWindow};
...
impl ManageEventsService {
    ...
    fn sanitize(mut cmd: UpsertEventCommand) -> Result<UpsertEventCommand, DomainError> {
        ...
        cmd.description = cmd.description.map(|d| d.trim().to_string()).filter(|d| !d.is_empty());
        cmd.game = cmd.game.map(|g| g.trim().to_string()).filter(|g| !g.is_empty());
        cmd.color = cmd
            .color
...
impl ManageEventsUseCase for ManageEventsService {
    async fn list_window(
        ...
        self.repo.list_in_window(guild_id, window, public_only).await
    }
