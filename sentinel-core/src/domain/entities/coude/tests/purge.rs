...
fn purge_order_bets_before_combats() {
    ...
    let bets = COUDE_PURGE_TABLES
        .iter()
        .position(|&t| t == "coude_bets")
        .unwrap();
    let combats = COUDE_PURGE_TABLES
        .iter()
        .position(|&t| t == "coude_combats")
        .unwrap();
...
fn purge_order_players_last() {
    ...
    let players = COUDE_PURGE_TABLES
        .iter()
        .position(|&t| t == "coude_players")
        .unwrap();
