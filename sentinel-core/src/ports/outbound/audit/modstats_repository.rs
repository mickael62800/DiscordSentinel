...
pub trait ModstatsRepository: Send + Sync {
    ...
    async fn breakdown(&self, _guild_id: &str, _days: i32, _limit: i64) -> Result<Vec<ModeratorBreakdown>, DomainError> {
        Ok(vec![])
    }

    /// Comptes quotidiens d'actions sur `days` jours (un point par jour).
    /// Default : vide (pour les stubs de test).
    async fn daily_trend(&self, _guild_id: &str, _days: i32) -> Result<Vec<ModstatsTrendDay>, DomainError> {
        Ok(vec![])
    }
}
