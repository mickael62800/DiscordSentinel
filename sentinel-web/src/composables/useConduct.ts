...
export function useConduct() {
  ...
  async function fetchConfig() {
    ...
    if (!guildId) return;
    try {
      config.value = await conductService.getConfig(guildId);
    } catch (e) {
  ...
  async function fetchLeaderboard() {
    ...
    loading.value = true;
    try {
      leaderboard.value = await conductService.getLeaderboard(guildId);
    } catch (e) {
  ...
  onMounted(() => { fetchConfig(); fetchLeaderboard(); });
  watch(selectedGuildId, () => { fetchConfig(); fetchLeaderboard(); });
...
export function useConductDetail() {
  ...
  async function fetchDetail(guildId: string, userId: string) {
    loading.value = true;
    try {
      points.value = await conductService.getPoints(guildId, userId);
      log.value = await conductService.getLog(guildId, userId);
    } catch (e) {
