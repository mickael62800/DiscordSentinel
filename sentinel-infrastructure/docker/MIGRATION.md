...
`sentinel-infrastructure/docker/`, ou ailleurs), Docker utilisera **toujours** le project name
...
docker compose -f sentinel-infrastructure/docker/docker-compose.yml build
```
...
```bash
docker compose -f sentinel-infrastructure/docker/docker-compose.yml \
...
docker compose -f sentinel-infrastructure/docker/docker-compose.yml down

# 3. Re-build et up
docker compose -f sentinel-infrastructure/docker/docker-compose.yml up -d --build

# 4. Surveille les logs sur les premieres minutes
docker compose -f sentinel-infrastructure/docker/docker-compose.yml logs -f --tail=50
```
...
```bash
docker compose -f sentinel-infrastructure/docker/docker-compose.yml --profile tls up -d
```
...
3. Vérifie que le compose a bien `name: discordsentinel` :
   ```bash
   grep "^name:" sentinel-infrastructure/docker/docker-compose.yml
...
| #10 docker-bake.hcl | ✅ | `sentinel-infrastructure/docker/docker-bake.hcl` |
