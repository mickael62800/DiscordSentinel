...
// ══════════════════════════════════════════════════════════
//  Conduct points (user_conduct_points)
// ══════════════════════════════════════════════════════════

#[tokio::test]
async fn conduct_points_deducted() {
    let pool = setup_pool().await;
    let gid = unique_guild();

    sqlx::query(
        r#"INSERT INTO user_conduct_points (id, guild_id, user_id, username, points, created_at, updated_at)
           VALUES (gen_random_uuid(), $1, '444444444444444444', 'User', 100, NOW(), NOW())"#,
    ).bind(&gid).execute(&pool).await.unwrap();

    sqlx::query(
        "UPDATE user_conduct_points SET points = GREATEST(0, points - $3) WHERE guild_id = $1 AND user_id = $2",
    ).bind(&gid).bind("444444444444444444").bind(30i32).execute(&pool).await.unwrap();

    let points = sqlx::query_as::<_, (i32,)>(
        "SELECT points FROM user_conduct_points WHERE guild_id = $1 AND user_id = '444444444444444444'",
    ).bind(&gid).fetch_one(&pool).await.unwrap().0;
...
#[tokio::test]
async fn conduct_points_cannot_go_negative() {
    let pool = setup_pool().await;
    let gid = unique_guild();

    sqlx::query(
        r#"INSERT INTO user_conduct_points (id, guild_id, user_id, username, points, created_at, updated_at)
           VALUES (gen_random_uuid(), $1, '444444444444444444', 'User', 10, NOW(), NOW())"#,
    ).bind(&gid).execute(&pool).await.unwrap();

    sqlx::query(
        "UPDATE user_conduct_points SET points = GREATEST(0, points - $3) WHERE guild_id = $1 AND user_id = $2",
    ).bind(&gid).bind("444444444444444444").bind(500i32).execute(&pool).await.unwrap();
