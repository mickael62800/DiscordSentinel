...
use sentinel_api::domain::entities::GuildMember;
use sentinel_api::ports::outbound::MemberRepository;
...
fn member(guild: &str, user: &str, name: &str) -> GuildMember {
    GuildMember {
        guild_id: guild.into(), user_id: user.into(), username: name.into(),
...
async fn find_one_not_found_returns_none() {
    let repo = PgMemberRepository::new(pool().await);
    assert!(repo.find_one(&fresh_id(), &fresh_id()).await.unwrap().is_none());
}
...
async fn find_by_guild_ordered_by_username() {
    let repo = PgMemberRepository::new(pool().await);
    let g = fresh_id();
    repo.upsert(&member(&g, &fresh_id(), "Charlie")).await.unwrap();
    repo.upsert(&member(&g, &fresh_id(), "Alice")).await.unwrap();
...
async fn update_last_seen_touches_timestamp() {
    ...
    assert!(after > before, "last_seen_at didn't advance: before={before:?} after={after:?}");
}
