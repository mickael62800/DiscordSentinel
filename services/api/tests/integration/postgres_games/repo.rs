...
async fn create_and_list_returns_game() {
    let repo = PgGameRepository::new(pool().await);
    let g = fresh_id();
    let created = repo.create(&g, "Minecraft", "admin1", None, None, None).await.unwrap();
...
async fn create_with_emoji_category_role() {
    let repo = PgGameRepository::new(pool().await);
    let g = fresh_id();
    let created = repo.create(&g, "Valorant", "admin1",
        Some("emoji"), Some("FPS"), Some("role1")).await.unwrap();
...
async fn find_by_name_returns_game() {
    let repo = PgGameRepository::new(pool().await);
    let g = fresh_id();
    repo.create(&g, "LoL", "admin1", None, None, None).await.unwrap();
    let got = repo.find_by_name(&g, "LoL").await.unwrap().unwrap();
    assert_eq!(got.game_name, "LoL");
    // Cas absent
    assert!(repo.find_by_name(&g, "NonExistent").await.unwrap().is_none());
}
...
async fn update_game_name() {
    let repo = PgGameRepository::new(pool().await);
    let g = fresh_id();
    let created = repo.create(&g, "OldName", "admin1", None, None, None).await.unwrap();
    let updated = repo.update(&g, &created.id, Some("NewName"), None, None).await.unwrap().unwrap();
...
async fn update_emoji_set_and_clear() {
    let repo = PgGameRepository::new(pool().await);
    let g = fresh_id();
    let created = repo.create(&g, "G", "admin1", None, None, None).await.unwrap();
    repo.update(&g, &created.id, None, Some(Some("emo")), None).await.unwrap();
    let got = repo.find_by_name(&g, "G").await.unwrap().unwrap();
    assert_eq!(got.emoji.as_deref(), Some("emo"));
    // Clear : Some(None)
    repo.update(&g, &created.id, None, Some(None), None).await.unwrap();
    let got = repo.find_by_name(&g, "G").await.unwrap().unwrap();
...
async fn update_not_found_returns_none() {
    ...
    let got = repo.update(&g, &bogus_id, Some("X"), None, None).await.unwrap();
    assert!(got.is_none());
}
...
async fn set_role_id_updates_and_clears() {
    ...
    let g = fresh_id();
    let created = repo.create(&g, "G", "admin1", None, None, None).await.unwrap();
    let got = repo.set_role_id(&g, &created.id, Some("role99")).await.unwrap().unwrap();
    assert_eq!(got.role_id.as_deref(), Some("role99"));
    let got = repo.set_role_id(&g, &created.id, None).await.unwrap().unwrap();
...
async fn list_by_category_filters() {
    let repo = PgGameRepository::new(pool().await);
    let g = fresh_id();
    repo.create(&g, "L", "a", None, Some("MOBA"), None).await.unwrap();
    repo.create(&g, "V", "a", None, Some("FPS"), None).await.unwrap();
    repo.create(&g, "M", "a", None, Some("FPS"), None).await.unwrap();
    ...
    repo.create(&g, "Uncategorized", "a", None, None, None).await.unwrap();
...
async fn save_and_find_panel_by_message() {
    let repo = PgGameRepository::new(pool().await);
    let g = fresh_id();
    let panel = repo.save_panel(&g, "chan1", "msg1", Some("MOBA")).await.unwrap();
    assert_eq!(panel.channel_id, "chan1");
    assert_eq!(panel.message_id, "msg1");

    let got = repo.find_panel_by_message(&g, "msg1").await.unwrap().unwrap();
    assert_eq!(got.channel_id, "chan1");
    assert!(repo.find_panel_by_message(&g, "nonexistent").await.unwrap().is_none());
}
