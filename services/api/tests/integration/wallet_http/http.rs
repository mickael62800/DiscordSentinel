...
use sentinel_api::domain::entities::casino::wallet::Wallet;
use sentinel_api::domain::entities::casino::wallet::WalletTransaction;
...
impl MockWalletRepo {
    ...
    fn with(self, w: Wallet) -> Self { self.wallets.lock().unwrap().push(w); self }
}
...
impl WalletRepository for MockWalletRepo {
    async fn get_or_create(&self, guild_id: &str, user_id: &str, _username: &str, starting_coins: i64) -> Result<Wallet, DomainError> {
        let mut wallets = self.wallets.lock().unwrap();
        if let Some(w) = wallets.iter().find(|w| w.guild_id == guild_id && w.user_id == user_id) {
            return Ok(w.clone());
        }
    ...
    async fn get(&self, guild_id: &str, user_id: &str) -> Result<Option<Wallet>, DomainError> {
        Ok(self.wallets.lock().unwrap().iter()
            .find(|w| w.guild_id == guild_id && w.user_id == user_id).cloned())
    }
    async fn credit(&self, guild_id: &str, user_id: &str, amount: i64, _: &str, _: &str) -> Result<Wallet, DomainError> {
        let mut wallets = self.wallets.lock().unwrap();
        let w = wallets.iter_mut()
            .find(|w| w.guild_id == guild_id && w.user_id == user_id);
        match w {
            Some(w) => { w.coins += amount; w.total_earned += amount; Ok(w.clone()) }
            None => {
    ...
    async fn debit(&self, guild_id: &str, user_id: &str, amount: i64, _: &str, _: &str) -> Result<Wallet, DomainError> {
        let mut wallets = self.wallets.lock().unwrap();
        let w = wallets.iter_mut()
            .find(|w| w.guild_id == guild_id && w.user_id == user_id)
    ...
    async fn transfer(&self, guild_id: &str, from: &str, to: &str, amount: i64, _: &str, _: &str) -> Result<(), DomainError> {
        self.transfers.lock().unwrap().push((guild_id.into(), from.into(), to.into(), amount));
        Ok(())
    }
    ...
    async fn leaderboard(&self, guild_id: &str, limit: i64) -> Result<Vec<Wallet>, DomainError> {
        ...
        let mut matching: Vec<Wallet> = wallets.iter()
            .filter(|w| w.guild_id == guild_id).cloned().collect();
        matching.sort_by(|a, b| b.coins.cmp(&a.coins));
        matching.truncate(limit as usize);
        Ok(matching)
    }
    async fn get_transactions(&self, _: &str, _: &str, _: i64) -> Result<Vec<WalletTransaction>, DomainError> {
    ...
    async fn reset_wallet(&self, guild_id: &str, user_id: &str, new_balance: i64) -> Result<Wallet, DomainError> {
        self.resets.lock().unwrap().push((guild_id.into(), user_id.into(), new_balance));
        let mut wallets = self.wallets.lock().unwrap();
        let w = wallets.iter_mut()
            .find(|w| w.guild_id == guild_id && w.user_id == user_id);
        match w {
            Some(w) => { w.coins = new_balance; w.total_earned = 0; w.total_spent = 0; Ok(w.clone()) }
            None => {
    ...
    async fn reset_all_wallets(&self, guild_id: &str, new_balance: i64) -> Result<u64, DomainError> {
        ...
        self.bulk_resets.lock().unwrap().push((guild_id.into(), new_balance, affected));
        Ok(affected)
    }
}
...
async fn get_wallet_returns_existing() {
    let repo = Arc::new(MockWalletRepo::new()
        .with(sample_wallet("111111111111111111", "444444444444444444", 500)));
...
async fn credit_success() {
    ...
    let (status, json) = post_json(app, "/api/wallet/111111111111111111/444444444444444444/credit", body).await;
    assert_eq!(status, StatusCode::OK);
...
async fn credit_negative_amount_422() {
    ...
    let (status, _) = post_json(app, "/api/wallet/111111111111111111/444444444444444444/credit", body).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
}
...
async fn debit_success() {
    let repo = Arc::new(MockWalletRepo::new()
        .with(sample_wallet("111111111111111111", "444444444444444444", 200)));
    let app = build_app(repo);
    let body = serde_json::json!({"amount": 50, "source": "buy", "description": "item"});
    let (status, json) = post_json(app, "/api/wallet/111111111111111111/444444444444444444/debit", body).await;
    assert_eq!(status, StatusCode::OK);
...
async fn debit_insufficient_funds_fails() {
    let repo = Arc::new(MockWalletRepo::new()
        .with(sample_wallet("111111111111111111", "444444444444444444", 10)));
    let app = build_app(repo);
    let body = serde_json::json!({"amount": 100, "source": "s", "description": "d"});
    let (status, _) = post_json(app, "/api/wallet/111111111111111111/444444444444444444/debit", body).await;
...
async fn leaderboard_respects_limit() {
    ...
    for i in 0..10 {
        repo = repo.with(sample_wallet("111111111111111111", &format!("u{i}"), i * 10));
    }
...
async fn transactions_empty() {
    let app = build_app(Arc::new(MockWalletRepo::new()));
    let (status, json) = get(app, "/api/wallet/111111111111111111/444444444444444444/transactions").await;
    assert_eq!(status, StatusCode::OK);
...
async fn reset_wallet_default_100() {
    ...
    let app = build_app(repo.clone());
    let body = serde_json::json!({});
    let (status, json) = post_json(app, "/api/wallets/111111111111111111/444444444444444444/reset", body).await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(json["coins"], 100);
