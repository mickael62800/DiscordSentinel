...
async fn create_ticket(pool: &PgPool, gid: &str, author: &str, title: &str, category: &str) -> uuid::Uuid {
    ...
    ).bind(title).bind(author).bind(gid).bind(category).fetch_one(pool).await.unwrap().0
}
...
async fn ticket_create_defaults() {
    ...
    let row = sqlx::query_as::<_, (String, String)>("SELECT status, priority FROM tickets WHERE id = $1")
...
async fn ticket_messages_cascade_delete() {
    ...
    let msgs = sqlx::query_as::<_, (i64,)>("SELECT COUNT(*) FROM ticket_messages WHERE ticket_id = $1")
        .bind(id).fetch_one(&p).await.unwrap().0;
    assert_eq!(msgs, 3);

    // Supprimer le ticket → messages supprimes en cascade
    sqlx::query("DELETE FROM tickets WHERE id = $1").bind(id).execute(&p).await.unwrap();
    let after = sqlx::query_as::<_, (i64,)>("SELECT COUNT(*) FROM ticket_messages WHERE ticket_id = $1")
        .bind(id).fetch_one(&p).await.unwrap().0;
    assert_eq!(after, 0);
}
...
async fn ticket_filter_by_status() {
    ...
    sqlx::query("UPDATE tickets SET status = 'closed' WHERE id = $1").bind(id3).execute(&p).await.unwrap();

    let open = sqlx::query_as::<_, (i64,)>("SELECT COUNT(*) FROM tickets WHERE server = $1 AND status = 'open'")
...
async fn ticket_assign_to_staff() {
    ...
    sqlx::query("UPDATE tickets SET assigned_to = '333', status = 'in_progress' WHERE id = $1")
        .bind(id).execute(&p).await.unwrap();
    let row = sqlx::query_as::<_, (Option<String>, String)>("SELECT assigned_to, status FROM tickets WHERE id = $1")
        .bind(id).fetch_one(&p).await.unwrap();
...
async fn ticket_satisfaction_rating() {
    ...
    sqlx::query("UPDATE tickets SET satisfaction_rating = 5, status = 'closed' WHERE id = $1")
        .bind(id).execute(&p).await.unwrap();
    let rating = sqlx::query_as::<_, (Option<i32>,)>("SELECT satisfaction_rating FROM tickets WHERE id = $1")
        .bind(id).fetch_one(&p).await.unwrap().0;
...
async fn ticket_first_response_tracked() {
    ...
    sqlx::query("UPDATE tickets SET first_response_at = NOW() WHERE id = $1")
        .bind(id).execute(&p).await.unwrap();
    let has_response = sqlx::query_as::<_, (bool,)>("SELECT first_response_at IS NOT NULL FROM tickets WHERE id = $1")
        .bind(id).fetch_one(&p).await.unwrap().0;
    assert!(has_response);
}
