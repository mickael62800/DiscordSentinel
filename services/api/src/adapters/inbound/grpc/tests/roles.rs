use super::*;
    ...
    impl ManageRolePanelsUseCase for MockPanelsUc {
        ...
        async fn get_panel(&self, _: &str) -> Result<RolePanelDetail, DomainError> {
            ...
            self.panel_detail.lock().unwrap().clone()
        ...
        async fn delete_panel(&self, _: &str) -> Result<(), DomainError> { Ok(()) }
        async fn list_auto_roles(&self, _: &str) -> Result<Vec<AutoRole>, DomainError> {
            Ok(self.auto_roles.lock().unwrap().clone())
        }
        async fn add_auto_role(&self, _: CreateAutoRoleCommand) -> Result<AutoRole, DomainError> { unimplemented!() }
        async fn delete_auto_role(&self, _: &str, _: &str) -> Result<(), DomainError> { Ok(()) }
    }
    ...
    fn grpc(uc: Arc<MockPanelsUc>, repo: Arc<MockRoleRepo>) -> RolePanelsGrpc {
        RolePanelsGrpc { uc, discord_role_repo: repo }
    }
    ...
    async fn get_panel_not_found_returns_none_not_error() {
        ...
        *uc.get_panel_not_found.lock().unwrap() = true;
        ...
        let resp = g.get_panel(Request::new(proto::GetPanelRequest {
    ...
    async fn get_panel_by_message_some_when_found() {
        ...
        *uc.detail_by_message.lock().unwrap() = Some(RolePanelDetail {
        ...
        let g = grpc(uc, Arc::new(MockRoleRepo::default()));
        let resp = g.get_panel_by_message(Request::new(proto::GetPanelByMessageRequest {
            message_id: "m".into(),
        })).await.unwrap();
        assert!(resp.into_inner().panel.is_some());
    }
    ...
    async fn list_panels_returns_all() {
        let uc = Arc::new(MockPanelsUc::default());
        uc.panels.lock().unwrap().extend(vec![sample_panel(), sample_panel()]);
        let g = grpc(uc, Arc::new(MockRoleRepo::default()));
        let resp = g.list_panels(Request::new(proto::ListPanelsRequest {
            guild_id: "g".into(),
        })).await.unwrap();
    ...
    async fn set_message_id_delegates_command_to_uc() {
        ...
        let _ = g.set_message_id(Request::new(proto::SetMessageIdRequest {
            panel_id: "p1".into(),
            message_id: "m1".into(),
        })).await.unwrap();
    ...
    async fn list_auto_roles_returns_list() {
        ...
        let g = grpc(uc, Arc::new(MockRoleRepo::default()));
        let resp = g.list_auto_roles(Request::new(proto::ListAutoRolesRequest {
            guild_id: "g".into(),
        })).await.unwrap();
    ...
    async fn sync_discord_roles_returns_synced_count() {
        ...
        let resp = g.sync_discord_roles(Request::new(proto::SyncDiscordRolesRequest {
            guild_id: "g".into(),
            roles: vec![
                proto::SyncDiscordRole {
                    id: "r1".into(), name: "A".into(), color: 0, position: 1,
                    permissions: "8".into(), mentionable: false, managed: false,
                    icon: None, member_count: 10,
                },
                proto::SyncDiscordRole {
                    id: "r2".into(), name: "B".into(), color: 0, position: 2,
                    permissions: "invalid".into(), mentionable: true, managed: true,
                    icon: Some("hash".into()), member_count: 0,
                },
            ],
        })).await.unwrap();
