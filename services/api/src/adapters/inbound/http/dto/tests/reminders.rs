...
fn from_dto_valid_uuid_preserved() {
    ...
    let cmd: crate::ports::inbound::CreateReminderCommand = make_dto(id).into();
    assert_eq!(cmd.action_id.to_string(), id);
}

#[test]
fn from_dto_invalid_uuid_falls_back_to_nil() {
    let cmd: crate::ports::inbound::CreateReminderCommand = make_dto("not-a-uuid").into();
    assert_eq!(cmd.action_id, Uuid::nil());
}

#[test]
fn from_dto_empty_uuid_falls_back_to_nil() {
    let cmd: crate::ports::inbound::CreateReminderCommand = make_dto("").into();
    assert_eq!(cmd.action_id, Uuid::nil());
}
...
fn from_dto_preserves_all_fields() {
    ...
    let cmd: crate::ports::inbound::CreateReminderCommand = dto.into();
