...
pub async fn grant_role(
    ...
    match res {
        ...
        Err(sqlx::Error::Database(db)) if db.is_unique_violation() => {
            Err(ApiError(crate::domain::errors::DomainError::ValidationError(
...
fn parse_role(s: &str) -> Result<Role, ApiError> {
    Role::from_str(s).ok_or_else(|| {
        ApiError(crate::domain::errors::DomainError::ValidationError(format!(
...
fn internal(msg: String) -> ApiError {
    ApiError(crate::domain::errors::DomainError::Internal(msg))
}

fn status_to_err(status: StatusCode, context: &str) -> ApiError {
    match status {
        StatusCode::FORBIDDEN => ApiError(crate::domain::errors::DomainError::Forbidden(
            context.to_string(),
        )),
        _ => ApiError(crate::domain::errors::DomainError::Internal(format!(
