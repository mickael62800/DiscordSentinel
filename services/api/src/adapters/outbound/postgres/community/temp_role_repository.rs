...
impl TempRoleRepository for PgTempRoleRepository {
    async fn create(&self, guild_id: &str, user_id: &str, role_id: &str, expires_at: &str) -> Result<(), crate::domain::errors::DomainError> {
        sqlx::query(
    ...
    async fn list_active(&self, guild_id: &str) -> Result<Vec<TempRole>, crate::domain::errors::DomainError> {
        ...
        Ok(rows.into_iter().map(|r| TempRole {
            ...
            role_id: r.role_id, expires_at: r.expires_at, created_at: r.created_at,
        }).collect())
    }

    async fn delete(&self, guild_id: &str, user_id: &str, role_id: &str) -> Result<(), crate::domain::errors::DomainError> {
