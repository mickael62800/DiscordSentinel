...
impl NotesRepository for PgNotesRepository {
    async fn save(&self, note: &UserNote) -> Result<(), DomainError> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| DomainError::Internal(format!("save_note: {e}")))?;
        Ok(())
    }
    ...
    async fn find_by_user(&self, guild_id: &str, user_id: &str) -> Result<Vec<UserNote>, DomainError> {
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| DomainError::Internal(format!("find_notes: {e}")))?;
    ...
    async fn delete(&self, note_id: &str) -> Result<(), DomainError> {
        ...
        sqlx::query("DELETE FROM user_notes WHERE id = $1")
            ...
            .execute(&self.pool)
            .await
            .map_err(|e| DomainError::Internal(format!("delete_note: {e}")))?;
        Ok(())
    }
}
