...
impl ReviewRepository for PgReviewRepository {
    async fn add(
        &self, action_id: uuid::Uuid, guild_id: &str, added_by: &str,
        added_by_name: &str, reason: Option<&str>,
    ) -> Result<ReviewEntry, crate::domain::errors::DomainError> {
        ...
        .bind(action_id).bind(guild_id).bind(added_by).bind(added_by_name).bind(reason)
        .fetch_one(&self.pool).await.map_err(pg_err)?;

        Ok(ReviewEntry {
            id: row.0, action_id, guild_id: guild_id.into(),
            added_by: added_by.into(), added_by_name: added_by_name.into(),
            reason: reason.map(|s| s.into()), status: "pending".into(),
            reviewer_id: None, reviewer_name: None, reviewer_notes: None,
            added_at: row.1, resolved_at: None,
            action_type: None, target_name: None, action_reason: None,
        })
    }

    async fn list_pending(&self, guild_id: &str) -> Result<Vec<ReviewEntry>, crate::domain::errors::DomainError> {
        #[derive(sqlx::FromRow)]
        struct Row {
            id: uuid::Uuid, action_id: uuid::Uuid,
            added_by: String, added_by_name: String,
            reason: Option<String>, added_at: chrono::DateTime<chrono::Utc>,
            action_type: String, target_name: String, action_reason: String,
        }
        ...
        Ok(rows.into_iter().map(|r| ReviewEntry {
            id: r.id, action_id: r.action_id, guild_id: guild_id.into(),
            added_by: r.added_by, added_by_name: r.added_by_name,
            reason: r.reason, status: "pending".into(),
            reviewer_id: None, reviewer_name: None, reviewer_notes: None,
            added_at: r.added_at, resolved_at: None,
            action_type: Some(r.action_type), target_name: Some(r.target_name),
            action_reason: Some(r.action_reason),
        }).collect())
    }

    async fn resolve(
        &self, review_id: uuid::Uuid, reviewer_id: &str, reviewer_name: &str,
        ...
        .bind(status).bind(reviewer_id).bind(reviewer_name).bind(notes).bind(review_id)
        .execute(&self.pool).await.map_err(pg_err)?;
        Ok(res.rows_affected() > 0)
    }

    async fn get_guild_id(&self, review_id: uuid::Uuid) -> Result<Option<String>, crate::domain::errors::DomainError> {
        let row: Option<(String,)> = sqlx::query_as(
            "SELECT guild_id FROM review_queue WHERE id = $1",
        )
        .bind(review_id).fetch_optional(&self.pool).await.map_err(pg_err)?;
        Ok(row.map(|r| r.0))
    }
}
