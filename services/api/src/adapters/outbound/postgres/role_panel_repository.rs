...
impl From<PanelRow> for RolePanel {
    fn from(r: PanelRow) -> Self {
        Self { id: r.id, guild_id: r.guild_id, channel_id: r.channel_id, message_id: r.message_id, title: r.title, description: r.description, mode: r.mode, max_roles: r.max_roles, enabled: r.enabled, created_at: r.created_at, updated_at: r.updated_at }
    }
}

impl From<EntryRow> for RolePanelEntry {
    fn from(r: EntryRow) -> Self {
        Self { id: r.id, panel_id: r.panel_id, role_id: r.role_id, role_name: r.role_name, emoji: r.emoji, label: r.label, style: r.style, position: r.position }
    }
}

impl From<AutoRoleRow> for AutoRole {
    fn from(r: AutoRoleRow) -> Self {
        Self { id: r.id, guild_id: r.guild_id, role_id: r.role_id, role_name: r.role_name, delay_secs: r.delay_secs, enabled: r.enabled }
    }
}
...
impl RolePanelRepository for PgRolePanelRepository {
    async fn save_panel(&self, panel: &RolePanel) -> Result<(), DomainError> {
        ...
        .bind(panel.id).bind(&panel.guild_id).bind(&panel.channel_id).bind(&panel.message_id)
    ...
    async fn save_entries(&self, entries: &[RolePanelEntry]) -> Result<(), DomainError> {
        for entry in entries {
            ...
            .bind(entry.id).bind(entry.panel_id).bind(&entry.role_id).bind(&entry.role_name)
    ...
    async fn save_auto_role(&self, ar: &AutoRole) -> Result<(), DomainError> {
        ...
        .bind(ar.id).bind(&ar.guild_id).bind(&ar.role_id).bind(&ar.role_name).bind(ar.delay_secs).bind(ar.enabled)
        .execute(&self.pool).await.map_err(|e| DomainError::Internal(e.to_string()))?;
        Ok(())
    }
