...
impl SponsorshipRepository for PgSponsorshipRepository {
    async fn create(&self, guild_id: &str, sponsor_id: &str, sponsored_id: &str) -> Result<(), crate::domain::errors::DomainError> {
        sqlx::query(
    ...
    async fn list(&self, guild_id: &str) -> Result<Vec<Sponsorship>, crate::domain::errors::DomainError> {
        ...
        Ok(rows.into_iter().map(|r| Sponsorship {
            id: r.id, guild_id: r.guild_id, sponsor_id: r.sponsor_id,
            sponsored_id: r.sponsored_id, created_at: r.created_at,
        }).collect())
    }
}
