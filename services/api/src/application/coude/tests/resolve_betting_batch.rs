...
struct MockCombatRepo {
    due: Mutex<Vec<CoudeCombat>>,
    stuck: Mutex<Vec<CoudeCombat>>,
...
impl CoudeCombatRepository for MockCombatRepo {
    ...
    async fn claim_due_betting_combats(&self, _: i64) -> Result<Vec<CoudeCombat>, DomainError> {
        Ok(std::mem::take(&mut *self.due.lock().unwrap()))
    }
    async fn claim_stuck_resolving_combats(&self, _: i64) -> Result<Vec<CoudeCombat>, DomainError> {
...
struct MockPlayerRepo {
    players: Mutex<std::collections::HashMap<String, CoudePlayer>>,
