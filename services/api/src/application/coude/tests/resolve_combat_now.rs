...
use crate::application::ResolveCombatNowService;
use crate::domain::entities::{
    BotDefinition, BotGuildConfig, CombatResolution, CombatStat, CoudeBet, CoudeCombat,
    CoudeCurrentSeason, CoudeEvent, CoudeInsurance, CoudeInventoryItem, CoudeLeaderboardEntry,
    CoudePlayer, CoudePrime, CoudeTauntsConfig, DailyChaosOutcome, LeaderboardCategory,
    NewCoudeBet, NewCoudeCombat, NewCoudePrime, NewDailyChaos, RefundSummary, TauntEvent, Wallet,
    WalletTransaction, XpProgress,
};
...
impl Default for MockCombatsUc {
    fn default() -> Self { Self { combat: Mutex::new(None), should_fail: false } }
}
...
impl ManageCoudePlayersUseCase for MockPlayersUc {
    ...
    async fn record_loss(&self, g: &str, u: &str, lost: i64) -> Result<(), DomainError> {
        self.record_loss_calls.lock().unwrap().push((g.into(), u.into(), lost));
        Ok(())
    }
    async fn record_draw(&self, g: &str, u: &str, lost: i64) -> Result<(), DomainError> {
        self.record_draw_calls.lock().unwrap().push((g.into(), u.into(), lost));
        Ok(())
    }
    ...
    async fn record_coins_earned(&self, g: &str, u: &str, a: i64) -> Result<(), DomainError> {
        self.record_earned_calls.lock().unwrap().push((g.into(), u.into(), a));
        Ok(())
    }
...
impl WalletRepository for MockWalletRepo {
    ...
    async fn get(&self, g: &str, u: &str) -> Result<Option<Wallet>, DomainError> {
        ...
        Ok(map.get(&format!("{g}:{u}")).map(|&coins| Wallet {
            ...
            coins, total_earned: 0, total_spent: 0,
    ...
    async fn pay_combat_atomic(&self, g: &str, winner: &str, winner_amount: i64, loser: &str, loser_amount: i64, source: &str, _: &str) -> Result<(), DomainError> {
        if winner_amount > 0 {
            self.credit_calls.lock().unwrap().push((g.into(), winner.into(), winner_amount, source.into()));
        }
        if loser_amount > 0 {
            self.debit_calls.lock().unwrap().push((g.into(), loser.into(), loser_amount, source.into()));
        }
        Ok(())
    }
...
fn build_service() -> (
    ...
    (svc, combat_repo, combats_uc, players_uc, wallet_repo, inventory_uc, taunts_uc)
}
...
async fn resolve_now_refuses_surprise_when_defender_has_explosion() {
    ...
    *inventory_uc.has_explosion.lock().unwrap() = true;
    ...
    // surprise_allow_defender_counter est true par default dans CoudeBalanceParams.
...
async fn resolve_now_calls_taunts_on_winner_and_loser() {
    ...
    *players_uc.defender.lock().unwrap() = Some(make_player("def", 10, 100));
    ...
    assert!(saw_winner_loser, "devrait voir winner+loser en 20 iterations");
}
