...
use crate::application::ManageRolePanelsService;
...
use crate::ports::inbound::manage_role_panels::{
    CreateAutoRoleCommand, CreateRolePanelCommand, CreateRolePanelEntryCommand,
    ManageRolePanelsUseCase, SetMessageIdCommand,
};
...
impl RolePanelRepository for MockRepo {
    async fn save_panel(&self, p: &RolePanel) -> Result<(), DomainError> {
        self.saved_panels.lock().unwrap().push(p.clone()); Ok(())
    }
    async fn save_entries(&self, e: &[RolePanelEntry]) -> Result<(), DomainError> {
        self.saved_entries.lock().unwrap().extend_from_slice(e); Ok(())
    }
    ...
    async fn update_message_id(&self, p: &str, m: &str) -> Result<(), DomainError> {
        self.message_id_updates.lock().unwrap().push((p.into(), m.into())); Ok(())
    }
    async fn delete_panel(&self, p: &str) -> Result<(), DomainError> {
        self.delete_panels.lock().unwrap().push(p.into()); Ok(())
    }
    ...
    async fn save_auto_role(&self, a: &AutoRole) -> Result<(), DomainError> {
        self.saved_auto_roles.lock().unwrap().push(a.clone()); Ok(())
    }
    async fn delete_auto_role(&self, g: &str, r: &str) -> Result<(), DomainError> {
        self.delete_auto_roles.lock().unwrap().push((g.into(), r.into())); Ok(())
    }
}
...
async fn create_panel_saves_panel_and_entries_with_uuids() {
    ...
    let cmd = CreateRolePanelCommand {
        ...
        mode: "unique".into(), max_roles: Some(3),
        entries: vec![
            CreateRolePanelEntryCommand {
                role_id: "r1".into(), role_name: "Role1".into(),
                emoji: None, label: "L1".into(),
                style: "primary".into(), position: 0,
            },
            CreateRolePanelEntryCommand {
                role_id: "r2".into(), role_name: "Role2".into(),
                emoji: Some("game".into()), label: "".into(),
...
async fn list_panels_returns_saved() {
    ...
    r.saved_panels.lock().unwrap().push(RolePanel {
        id: uuid::Uuid::new_v4(), guild_id: "g".into(), channel_id: "c".into(),
        message_id: None, title: "t".into(), description: "d".into(),
        mode: "unique".into(), max_roles: None, enabled: true,
...
async fn set_message_id_forwards() {
    ...
    svc.set_message_id(SetMessageIdCommand {
        panel_id: "p1".into(), message_id: "m1".into()
    }).await.unwrap();
    assert_eq!(r.message_id_updates.lock().unwrap()[0], ("p1".into(), "m1".into()));
}
...
async fn add_auto_role_creates_enabled_with_uuid() {
    let r = Arc::new(MockRepo::default());
    let svc = ManageRolePanelsService::new(r.clone());
    let auto = svc.add_auto_role(CreateAutoRoleCommand {
        guild_id: "g".into(), role_id: "r".into(),
        role_name: "Welcome".into(), delay_secs: 60,
    }).await.unwrap();
...
async fn delete_auto_role_forwards_guild_and_role() {
    ...
    assert_eq!(r.delete_auto_roles.lock().unwrap()[0], ("g".into(), "r1".into()));
}
