...
fn sample_stats(guild: &str, user: &str, msgs: u64, voice: u64) -> UserStats {
    UserStats {
        ...
        username: user.into(), message_count: msgs, voice_seconds: voice,
        updated_at: Utc::now(),
    }
}
...
impl StatsRepository for MockStatsRepo {
    async fn upsert(&self, _: &UserStats) -> Result<(), DomainError> { Ok(()) }
    ...
    async fn increment_messages(&self, g: &str, u: &str, n: &str, c: u64) -> Result<(), DomainError> {
        self.inc_messages.lock().unwrap().push((g.into(), u.into(), n.into(), c));
        Ok(())
    }
    async fn add_voice_seconds(&self, g: &str, u: &str, n: &str, s: u64) -> Result<(), DomainError> {
        self.add_voice.lock().unwrap().push((g.into(), u.into(), n.into(), s));
        Ok(())
    }
    async fn count_distinct_guilds(&self) -> Result<u64, DomainError> { Ok(3) }
    async fn count_distinct_users(&self) -> Result<u64, DomainError> { Ok(25) }
    async fn save_voice_session(&self, g: &str, u: &str, n: &str, c: &str, cn: &str, d: u64)
        -> Result<(), DomainError>
    {
        self.voice_sessions.lock().unwrap().push((g.into(), u.into(), n.into(), c.into(), cn.into(), d));
        Ok(())
    }
    async fn get_guild_voice_stats(&self, _: &str, _: u32, _: u32)
...
fn sample_inf(action: Action) -> Infraction {
    Infraction {
        ...
        user_id: "u".into(), username: "u".into(), message_id: "m".into(),
        content: "".into(), flags: DetectionFlags { spam: false, insult: false, link: false, phishing: false },
        score: 0.0, action, reason: "".into(), duration: None,
        created_at: Utc::now(),
    }
}
#[async_trait]
impl InfractionRepository for MockInfractionRepo {
    async fn save(&self, _: &Infraction) -> Result<(), DomainError> { Ok(()) }
    async fn find_by_guild(&self, _: &str, _: &InfractionFilters) -> Result<Vec<Infraction>, DomainError> {
        Ok(self.infractions.lock().unwrap().clone())
    }
    async fn find_all(&self, _: i64, _: i64) -> Result<Vec<Infraction>, DomainError> { Ok(vec![]) }
    async fn count_today(&self) -> Result<u64, DomainError> { Ok(7) }
...
impl CachePort for MockCache {
    ...
    async fn set_json(&self, key: &str, json: &str, _: u64) -> Result<(), DomainError> {
        self.json_store.lock().unwrap().insert(key.into(), json.into());
        Ok(())
    }
...
async fn record_messages_increments_and_invalidates_overview() {
    ...
    let svc = make_service(s.clone(), Arc::new(MockInfractionRepo::default()), c.clone());
    svc.record_messages(RecordMessagesCommand {
        guild_id: "g1".into(), user_id: "u1".into(),
        username: "Alice".into(), count: 5,
    }).await.unwrap();
    assert_eq!(s.inc_messages.lock().unwrap()[0], ("g1".into(), "u1".into(), "Alice".into(), 5));
...
async fn record_voice_adds_seconds_saves_session_and_invalidates_all_periods() {
    ...
    svc.record_voice(RecordVoiceCommand {
        guild_id: "g1".into(), user_id: "u1".into(),
        username: "Alice".into(), channel_id: "chan".into(),
        channel_name: "general".into(), seconds: 120,
    }).await.unwrap();
...
async fn record_voice_empty_channel_skips_session_save() {
    let s = Arc::new(MockStatsRepo::default());
    let svc = make_service(s.clone(),
        Arc::new(MockInfractionRepo::default()),
        Arc::new(MockCache::default()));
    svc.record_voice(RecordVoiceCommand {
        guild_id: "g1".into(), user_id: "u1".into(),
        username: "Alice".into(), channel_id: "".into(),
        channel_name: "".into(), seconds: 60,
    }).await.unwrap();
...
async fn get_user_stats_forwards() {
    ...
    *s.find_by_user_returns.lock().unwrap() = Some(sample_stats("g", "u", 10, 50));
    let svc = make_service(s,
        Arc::new(MockInfractionRepo::default()), Arc::new(MockCache::default()));
...
async fn get_guild_overview_aggregates_and_counts_infractions_by_action() {
    ...
    *inf.infractions.lock().unwrap() = vec![
        sample_inf(Action::Warn), sample_inf(Action::Warn),
        sample_inf(Action::Mute), sample_inf(Action::Ban),
    ];
...
async fn get_guild_overview_is_cached() {
    ...
    let c = Arc::new(MockCache::default());
    let svc = make_service(s.clone(),
        Arc::new(MockInfractionRepo::default()), c.clone());
    let _ = svc.get_guild_overview("g").await.unwrap();
    let _ = svc.get_guild_overview("g").await.unwrap();
    // 2 appels au service, 1 seul en DB grace au cache.
    assert!(c.json_store.lock().unwrap().contains_key("stats:overview:g"));
}
...
async fn get_guild_voice_stats_aggregates_channels() {
    ...
    *s.voice_stats_returns.lock().unwrap() = vec![
        VoiceSessionStats {
            channel_id: "c1".into(), channel_name: "n1".into(),
            is_temporary: false, total_sessions: 10,
            total_duration_secs: 600, unique_users: 2,
            avg_duration_secs: 60, last_activity: None,
        },
        VoiceSessionStats {
            channel_id: "c2".into(), channel_name: "n2".into(),
            is_temporary: true, total_sessions: 5,
            total_duration_secs: 200, unique_users: 1,
            avg_duration_secs: 40, last_activity: None,
        },
    ];
