...
fn sample(id: &str) -> Infraction {
    Infraction {
        ...
        score: 0.0, action: Action::Warn, reason: "".into(), duration: None,
        created_at: Utc::now(),
    }
}
...
impl InfractionRepository for MockRepo {
    ...
    async fn find_by_guild(&self, g: &str, f: &InfractionFilters) -> Result<Vec<Infraction>, DomainError> {
        self.list_calls.lock().unwrap().push((g.into(), f.limit, f.offset));
    ...
    async fn count_today(&self) -> Result<u64, DomainError> { Ok(13) }
...
async fn list_infractions_forwards_filters() {
    ...
    let f = InfractionFilters { user_id: None, action: None, limit: 25, offset: 5 };
