...
const WORKER_NAME: &str = "coude-worker";

#[tokio::main]
async fn main() {
    common::init_tracing("sentinel_coude_worker=info");
    common::metrics::init_observability(WORKER_NAME);

    let mut config = WorkerConfig::from_env();

    info!("Demarrage de Sentinel Coude Worker");
    ...
    common::send_lifecycle_log(&config.api_url, WORKER_NAME, "info", "Coude Worker demarre").await;

    info!("Sentinel Coude Worker pret");

    common::shutdown_signal().await;

    common::send_lifecycle_log(&config.api_url, WORKER_NAME, "warn", "Coude Worker en cours d'arret").await;

    info!("Arret en cours...");
    let _ = shutdown_tx.send(true);

    pg_pool.close().await;
    info!("Sentinel Coude Worker arrete proprement");
}
