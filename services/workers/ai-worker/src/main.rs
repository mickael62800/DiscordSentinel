...
const WORKER_NAME: &str = "ai-worker";

#[tokio::main]
async fn main() {
    common::init_tracing("sentinel_ai_worker=info");
    common::metrics::init_observability(WORKER_NAME);

    let mut config = WorkerConfig::from_env();

    info!("Demarrage de Sentinel AI Worker");
    ...
    common::send_lifecycle_log(&config.api_url, WORKER_NAME, "info", "AI Worker demarre").await;

    info!("Sentinel AI Worker pret");

    common::shutdown_signal().await;

    common::send_lifecycle_log(&config.api_url, WORKER_NAME, "warn", "AI Worker en cours d'arret").await;

    info!("Arret en cours...");
    let _ = shutdown_tx.send(true);

    pg_pool.close().await;
    info!("Sentinel AI Worker arrete proprement");
}
