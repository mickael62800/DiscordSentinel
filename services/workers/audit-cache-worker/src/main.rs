...
const WORKER_NAME: &str = "audit-cache-worker";

#[tokio::main]
async fn main() {
    common::init_tracing("sentinel_audit_cache_worker=info");
    common::metrics::init_observability(WORKER_NAME);

    let mut config = WorkerConfig::from_env();

    info!("Demarrage de Sentinel Audit Cache Worker");
    ...
    common::send_lifecycle_log(&config.api_url, WORKER_NAME, "info", "Audit Cache Worker demarre").await;

    info!("Sentinel Audit Cache Worker pret");

    common::shutdown_signal().await;

    common::send_lifecycle_log(&config.api_url, WORKER_NAME, "warn", "Audit Cache Worker en cours d'arret").await;

    info!("Arret en cours...");
    let _ = shutdown_tx.send(true);

    pg_pool.close().await;
    info!("Sentinel Audit Cache Worker arrete proprement");
}
