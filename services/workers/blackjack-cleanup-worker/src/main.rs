...
const WORKER_NAME: &str = "blackjack-cleanup-worker";

#[tokio::main]
async fn main() {
    common::init_tracing("sentinel_blackjack_cleanup_worker=info");
    common::metrics::init_observability(WORKER_NAME);

    let mut config = WorkerConfig::from_env();

    info!("Demarrage de Sentinel Blackjack Cleanup Worker");
    ...
    common::send_lifecycle_log(
        ...
        WORKER_NAME,
        "info",
        "Blackjack Cleanup Worker demarre",
    )
    .await;

    info!("Sentinel Blackjack Cleanup Worker pret");
    ...
    common::send_lifecycle_log(
        ...
        WORKER_NAME,
        "warn",
        "Blackjack Cleanup Worker en cours d'arret",
    )
    .await;
    ...
    pg_pool.close().await;
    info!("Sentinel Blackjack Cleanup Worker arrete proprement");
}
