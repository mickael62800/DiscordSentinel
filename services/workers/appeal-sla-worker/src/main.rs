...
const WORKER_NAME: &str = "appeal-sla-worker";

#[tokio::main]
async fn main() {
    common::init_tracing("sentinel_appeal_sla_worker=info");
    common::metrics::init_observability(WORKER_NAME);

    let mut config = WorkerConfig::from_env();

    info!("Demarrage de Sentinel Appeal SLA Worker");
    ...
    common::send_lifecycle_log(&config.api_url, WORKER_NAME, "info", "Appeal SLA Worker demarre").await;

    info!("Sentinel Appeal SLA Worker pret");

    common::shutdown_signal().await;

    common::send_lifecycle_log(&config.api_url, WORKER_NAME, "warn", "Appeal SLA Worker en cours d'arret").await;

    info!("Arret en cours...");
    let _ = shutdown_tx.send(true);

    pg_pool.close().await;
    info!("Sentinel Appeal SLA Worker arrete proprement");
}
